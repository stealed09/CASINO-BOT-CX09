...
async def cb_pvp_create(callback: CallbackQuery):
    ...
    from aiogram.types import InlineKeyboardButton
    builder = InlineKeyboardBuilder()
    for amt in [50, 100, 250, 500, 1000, 2500, 5000, 10000]:
        builder.button(text=f"{amt} T", callback_data=f"pvp_bet_{game_type}_{amt}")
    builder.adjust(4)
    builder.row(InlineKeyboardButton(text="🔙 Back", callback_data="menu_pvp"))
    try:
        await callback.message.edit_text(
...
@dp.callback_query(F.data.startswith("pvp_bet_"))
async def cb_pvp_bet_select(callback: CallbackQuery):
    parts = callback.data.split("_")
    bet = float(parts[-1])
    game_type = "_".join(parts[2:-1])
    user_id = callback.from_user.id
    user = await db.get_user(user_id)
    if not user:
        await callback.answer("Please /start the bot first!", show_alert=True); return
    if user["token_balance"] < bet:
        await callback.answer(f"Need {bet:,.0f} T. You have {user['token_balance']:,.0f}.", show_alert=True); return
    house_fee = float(await db.get_setting("pvp_house_fee") or "5")
    chat_id = callback.message.chat.id
    player_name = callback.from_user.first_name or str(user_id)
    duel_id = create_duel(user_id, player_name, game_type, bet, house_fee, chat_id)
    await db.update_token_balance(user_id, -bet)
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    builder = InlineKeyboardBuilder()
    ...
    if group_id and str(chat_id) != str(group_id):
        try:
