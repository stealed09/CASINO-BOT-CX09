...
def deposit_method_kb() -> InlineKeyboardMarkup:
    """Top-level deposit: choose INR, Stars, or Crypto."""
    builder = InlineKeyboardBuilder()
    builder.row(
...
def crypto_manual_select_kb(cryptos: List[Dict]) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for c in cryptos:
        builder.row(InlineKeyboardButton(
            text=f"₿ {c['symbol']} ({c['network']})",
            callback_data=f"crypto_deposit_manual_{c['symbol']}"
        ))
    builder.row(InlineKeyboardButton(text="🔙 Back", callback_data="wallet_deposit"))
    return builder.as_markup()
