...
def leaderboard_text(entries: list, period: str, fake_entries: list = None) -> str:
    """Merge real users (show only username) with fake entries, sort by wager desc."""
    period_names = {
    ...
    for u in (entries or []):
        ...
        uname = u.get("username") or f"User{str(u.get('user_id', '???'))[-4:]}"
        combined.append({"name": f"@{uname}", "wager": wager, "real": True})
    for f in (fake_entries or []):
        combined.append({"name": f["display_name"], "wager": f["total_wagered"], "real": False})

    combined.sort(key=lambda x: x["wager"], reverse=True)

    for i, entry in enumerate(combined[:10]):
        medal = medals[i] if i < 3 else f"{i+1}."
        lines.append(f"{medal} {entry['name']} — <b>{entry['wager']:,.0f}</b> Tokens")
