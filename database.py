...
class Database:
    ...
    async def update_wagered(self, user_id: int, amount: float):
        ...
        async with self._lock:
            ...
            await self.update_mission_progress(user_id, wager=amount, games_played=1)
    ...
    async def add_transaction(self, user_id: int, type_: str, amount: float, status: str = "completed", currency: str = "TOKEN"):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.commit()
        # Auto-track game wins/losses for profile stats
        if "_win" in type_ or type_ in ("crash_win", "slots_win", "pvp_win"):
            await self.record_game_result(user_id, True, amount)
            await self.update_mission_progress(user_id, wins=1)
        elif "_loss" in type_ or type_ in ("crash_loss", "slots_loss", "pvp_loss"):
