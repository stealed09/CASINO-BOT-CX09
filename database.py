...
class Database:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            for col, default in [
                ...
                try:
                    ...
                    pass
            # Add missing mission_progress columns for existing DBs
            for mp_col in ["limbo_2x","login","deposit","limbo_777","win_streak","comeback","refer","login_streak","vip_rankup"]:
                try:
                    await db.execute(f"ALTER TABLE mission_progress ADD COLUMN {mp_col} INTEGER DEFAULT 0")
                except:
                    pass
            ...
            # Mission settings — default all ON
    ...
    async def update_wagered(self, user_id: int, amount: float):
        ...
        async with self._lock:
            ...
            await self.increment_mission_field(user_id, _today, "wager", amount)
            await self.increment_mission_field(user_id, _today, "games_played")
            await self.increment_mission_field(user_id, _today, "login")
    ...
    async def record_game_result(self, user_id: int, won: bool, amount: float):
        ...
        _today = _date.today().isoformat()
        if won:
            await self.increment_mission_field(user_id, _today, "wins")
        await self.increment_mission_field(user_id, _today, "games_played")
    ...
    async def get_mission_progress_v2(self, user_id: int, period_key: str) -> dict:
        """Get mission progress for a specific period."""
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                "SELECT * FROM mission_progress WHERE user_id=? AND date=?",
                (user_id, period_key)
            ) as cur:
                row = await cur.fetchone()
            if not row:
            ...
            d = dict(row)
            import json
            d["claimed_missions"] = json.loads(d.get("claimed_missions") or "[]")
            return d

    async def get_all_mission_progress(self, user_id: int) -> dict:
        """Get merged progress across all period types."""
        from missions import get_period_key
        import json
        merged = {"claimed_missions": [], "wager": 0, "games_played": 0, "wins": 0,
                  "crash_plays": 0, "limbo_2x": 0, "login": 0, "deposit": 0,
                  "limbo_777": 0, "win_streak": 0, "comeback": 0,
                  "refer": 0, "login_streak": 0, "vip_rankup": 0}
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
        ...
        for row in rows:
            d = dict(row)
            for k in ["wager","games_played","wins","crash_plays","limbo_2x",
                      "login","deposit","limbo_777","win_streak","comeback",
                      "refer","login_streak","vip_rankup"]:
                if k in d:
                    merged[k] = merged.get(k, 0) + (d[k] or 0)
            claimed = json.loads(d.get("claimed_missions") or "[]")
            for c in claimed:
                if c not in merged["claimed_missions"]:
                    merged["claimed_missions"].append(c)
        return merged

    async def add_mission_claim(self, user_id: int, period_key: str, mission_id: str):
        import json
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                "SELECT claimed_missions FROM mission_progress WHERE user_id=? AND date=?",
                (user_id, period_key)
            ) as cur:
                row = await cur.fetchone()
            if row:
                claimed = json.loads(row[0] or "[]")
                if mission_id not in claimed:
                    claimed.append(mission_id)
                await db.execute(
                    "UPDATE mission_progress SET claimed_missions=? WHERE user_id=? AND date=?",
                    (json.dumps(claimed), user_id, period_key)
                )
            else:
                await db.execute(
                    "INSERT INTO mission_progress (user_id, date, claimed_missions) VALUES (?,?,?)",
                    (user_id, period_key, json.dumps([mission_id]))
                )
            await db.commit()

    async def increment_mission_field(self, user_id: int, period_key: str, field: str, amount: float = 1):
        """Increment a specific mission progress field."""
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("""
                INSERT INTO mission_progress (user_id, date) VALUES (?,?)
                ON CONFLICT(user_id, date) DO NOTHING
            """, (user_id, period_key))
            valid_fields = ["wager","games_played","wins","crash_plays","limbo_2x",
                          "login","deposit","limbo_777","win_streak","comeback",
                          "refer","login_streak","vip_rankup"]
            if field not in valid_fields:
                return
            await db.execute(
                f"UPDATE mission_progress SET {field}={field}+? WHERE user_id=? AND date=?",
                (amount, user_id, period_key)
            )
            await db.commit()

    # ─── LOSER LEADERBOARD ────────────────────────────────────────────────────

    async def get_top_losers(self, limit: int = 10) -> list:
        """Get users with most total losses."""
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("""
                SELECT u.username, u.user_id,
                       COALESCE(SUM(CASE WHEN t.type LIKE '%_loss' THEN t.amount ELSE 0 END), 0) as total_lost
                FROM users u
    ...
    async def get_enabled_missions(self) -> list:
        """Return list of enabled mission IDs."""
        from missions import MISSIONS
        enabled = []
        for m in MISSIONS:
            val = await self.get_setting(m["setting_key"])
            if val != "0":  # Default ON unless explicitly set to 0
                enabled.append(m["id"])
        return enabled
