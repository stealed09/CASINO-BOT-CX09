...
from aiogram.types import Message, CallbackQuery, PreCheckoutQuery, BotCommand, BotCommandScopeAllGroupChats, BotCommandScopeAllPrivateChats
...
# ─── GROUP GUARD — ignore non-command messages from groups ────────────────────
# This prevents the bot from reacting to every message in a group chat.
# Only messages that are actual bot commands will pass through in groups.

@dp.message(F.chat.type.in_({"group", "supergroup"}), ~F.text.startswith("/"))
async def _group_non_command_guard(message: Message, state: FSMContext):
    # In groups, silently ignore all non-command messages
    # EXCEPT if user is in an active FSM state (e.g. mid-game)
    current_state = await state.get_state()
    if current_state is None:
        return  # Not in any FSM flow — ignore
    # If they are in FSM flow, let it fall through to the right handler
...
async def main():
    ...
    # ── Register bot commands (shows in slash menu) ────────────────────
    private_commands = [
        ...
        BotCommand(command="limbo",      description="Play Limbo 🚀"),
        BotCommand(command="cf",         description="Coin Flip 🪙"),
        BotCommand(command="ping",       description="Bot status & latency 📡"),
    ]
    group_commands = [
        BotCommand(command="balance",    description="Check your balance 💰"),
        BotCommand(command="deposit",    description="Deposit tokens 💳"),
