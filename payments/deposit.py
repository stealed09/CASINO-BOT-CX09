...
async def _get_promo_preview(user_id: int, net: float) -> str:
    try:
        from promo import validate_promo
        code = await db.get_setting(f"pending_promo_{user_id}")
        if code and code.strip():
            valid, _, bonus = await validate_promo(code.strip(), user_id, net)
            if valid and bonus > 0:
                return (f"\n🎁 Promo Bonus: +<b>{bonus:,.4f} Tokens</b>"
                        f"\n✅ Total You Get: <b>{net+bonus:,.4f} Tokens</b>")
    except:
        pass
    return ""

async def _apply_promo(user_id: int, net: float) -> tuple:
    bonus, msg = 0.0, ""
    try:
        ...
        if code and code.strip():
            ...
            if valid and bonus > 0:
                ...
                await use_promo(code.strip(), user_id)
                await db.set_setting(f"used_promo_{user_id}", code.strip())
                msg = f"\n🎁 Promo <code>{code.strip()}</code>: +<b>{bonus:,.4f} Tokens</b>"
...
async def handle_successful_payment(message: Message, bot: Bot):
    ...
    await message.answer(
        success_text(
            ...
            f"📊 Rate: 1 ⭐ = <b>{rate} Tokens</b>\n"
            f"🪙 Gross: <b>{gross:,.4f}</b> | Tax: <b>-{tax:,.4f}</b>\n"
...
async def start_oxapay_deposit(callback: CallbackQuery, token_amount: float):
    ...
    dep_tax = await db.get_effective_deposit_tax(callback.from_user.id)
    promo_preview = await _get_promo_preview(callback.from_user.id, token_amount * (1 - dep_tax/100))
    try:
        await callback.message.edit_text(
    ...
    try:
        await bot.send_message(
            ...
            success_text(
                ...
                f"🪙 Gross: {gross:,.4f} | Tax ({dep_tax_pct}%): -{tax:,.4f}\n"
