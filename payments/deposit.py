...
async def _apply_promo(user_id: int, net_tokens: float) -> tuple[float, str]:
    """Check pending promo and apply bonus. Returns (bonus_tokens, promo_msg)."""
    bonus = 0.0
    msg = ""
    try:
        ...
        if pending_code and pending_code.strip():
            valid, pmsg, bonus = await validate_promo(pending_code, user_id, net_tokens)
...
async def start_upi_deposit(message: Message, bot: Bot, inr_amount: float):
    ...
    # Check if promo pending — show promo bonus in preview
    promo_preview = ""
    try:
        from promo import validate_promo, get_promo
        pending_code = await db.get_setting(f"pending_promo_{user_id}")
        if pending_code and pending_code.strip():
            valid, _, bonus = await validate_promo(pending_code, user_id, net)
            if valid and bonus > 0:
