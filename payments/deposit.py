...
async def start_upi_deposit(chat_id: int, bot: Bot, inr_amount: float, user_id: int, state=None):
    ...
    # Store did in FSM state so cb_upi_done can pick it up
    if state:
        await state.update_data(upi_did=did)

    promo_preview = await _get_promo_preview(user_id, net)
