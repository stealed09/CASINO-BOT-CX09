...
async def process_upi_withdrawal_form(message: Message, bot: Bot,
    ...
    # Check promo min withdrawal
    try:
        from promo import get_user_promo_min_withdraw
        promo_min_wd = await get_user_promo_min_withdraw(user_id)
        if promo_min_wd > 0 and token_amount < promo_min_wd:
            await message.answer(
                error_text(f"⚠️ Your promo bonus requires a minimum withdrawal of {promo_min_wd:,.2f} Tokens!"),
