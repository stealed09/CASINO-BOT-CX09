...
async def process_upi_withdrawal(message: Message, bot: Bot, token_amount: float, upi_id: str):
    ...
    if not user:
        await message.answer(error_text("Please /start first."), parse_mode="Markdown")
        return

    if user.get("is_banned"):
        await message.answer(error_text("Your account is banned."), parse_mode="Markdown")
        return
    ...
    await message.answer(
        success_text(
            ...
            f"🧾 Tax ({wd_tax_pct}%): *-{tax:,.4f}*\n"
            f"💵 Net (paid out): *{after_tax:,.4f} Tokens worth*\n"
            f"🏦 UPI: `{upi_id}`\n"
            f"🆔 ID: #{wid}\n"
...
async def process_crypto_withdrawal(message: Message, bot: Bot,
    ...
    withdraw_enabled = await db.get_setting("withdraw_enabled")
    if withdraw_enabled != "1":
        await message.answer(error_text("Withdrawals disabled."), parse_mode="Markdown", reply_markup=back_kb())
        return
