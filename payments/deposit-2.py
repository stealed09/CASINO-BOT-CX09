...
async def approve_deposit(callback: CallbackQuery, bot: Bot, did: int):
    ...
    try:
        ...
        if pending_code:
            valid, pmsg, promo_bonus = await validate_promo(pending_code, user_id, net)
            if valid and promo_bonus > 0:
                await db.update_token_balance(user_id, promo_bonus)
                await db.add_transaction(user_id, "promo_bonus", promo_bonus, currency="TOKEN")
                await use_promo(pending_code, user_id)
                await db.set_setting(f"used_promo_{user_id}", pending_code)
                promo_msg = f"\n🎁 Promo <code>{pending_code}</code>: +<b>{promo_bonus:,.4f} Tokens</b>"
            await db.set_setting(f"pending_promo_{user_id}", "")
    except Exception as pe:
        logger.error(f"Promo apply error: {pe}")
