...
async def _get_promo_preview(user_id: int, net_tokens: float) -> str:
    """Return promo bonus preview string for QR caption."""
    try:
        from promo import validate_promo
        pending_code = await db.get_setting(f"pending_promo_{user_id}")
        if pending_code and pending_code.strip():
            valid, _, bonus = await validate_promo(pending_code, user_id, net_tokens)
            if valid and bonus > 0:
                return f"\n🎁 Promo Bonus: +<b>{bonus:,.4f} Tokens</b>\n✅ Total: <b>{net_tokens + bonus:,.4f} Tokens</b>"
    except:
        pass
    return ""


async def _apply_promo(user_id: int, net_tokens: float) -> tuple:
    """Apply pending promo and return (bonus, msg)."""
    bonus = 0.0
    msg = ""
    try:
        from promo import validate_promo, use_promo
        pending_code = await db.get_setting(f"pending_promo_{user_id}")
        if pending_code and pending_code.strip():
            valid, _, bonus = await validate_promo(pending_code, user_id, net_tokens)
            if valid and bonus > 0:
                await db.update_token_balance(user_id, bonus)
                await db.add_transaction(user_id, "promo_bonus", bonus, currency="TOKEN")
                await use_promo(pending_code, user_id)
                await db.set_setting(f"used_promo_{user_id}", pending_code)
                msg = f"\n🎁 Promo <code>{pending_code}</code>: +<b>{bonus:,.4f} Tokens</b>"
            await db.set_setting(f"pending_promo_{user_id}", "")
    except Exception as e:
        logger.error(f"Promo apply error: {e}")
    return bonus, msg
...
async def start_upi_deposit(message_or_chat, bot: Bot, inr_amount: float, user_id: int = None):
    # Accept both Message and chat_id int
    if isinstance(message_or_chat, int):
        chat_id = message_or_chat
        uid = user_id or chat_id
    else:
        chat_id = message_or_chat.chat.id
        uid = user_id or message_or_chat.from_user.id

    upi_id = await db.get_setting("upi_id") or "notset@upi"
    gross, tax, net, dep_tax_pct = await tokens_for_inr(uid, inr_amount)
    rate = await get_inr_token_rate()
    did = await db.create_deposit(uid, "upi", inr_amount=inr_amount)

    # Show promo bonus in caption
    promo_preview = await _get_promo_preview(uid, net)
    ...
    caption = (
        ...
        f"🪙 Gross: <b>{gross:,.4f}</b> | Tax ({dep_tax_pct}%): <b>-{tax:,.4f}</b>\n"
...
async def handle_successful_payment(message: Message, bot: Bot):
    ...
    bonus, promo_msg = await _apply_promo(user_id, net)
    ...
    await message.answer(
        success_text(
            ...
            f"✅ Tokens Credited: <b>{net:,.4f}</b>" + promo_msg
        ),
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"⭐ {stars_paid} Stars → 🪙 {net:,.4f} Tokens" + promo_msg,
                parse_mode="HTML"
            )
        except:
            pass
...
async def create_oxapay_deposit(message: Message, bot: Bot,
    ...
    rate = await get_usd_token_rate()
    dep_tax_pct = await db.get_effective_deposit_tax(user_id)
    usd_amount = max(round(token_amount / rate, 2), 1.0)
    ...
    promo_preview = await _get_promo_preview(user_id, net_tokens)
...
async def poll_oxapay_status(bot: Bot, track_id: str, did: int, user_id: int, net_tokens: float):
    for _ in range(180):
        ...
        if status in PAID_STATUSES:
            ...
            bonus, promo_msg = await _apply_promo(user_id, net_tokens)
            try:
                await bot.send_message(
                    user_id,
                    success_text(
                        f"₿ Crypto Payment Confirmed!\n"
                        f"✅ Tokens Credited: <b>{net_tokens:,.4f}</b>" + promo_msg
                    ),
                    parse_mode="HTML"
                )
            except:
...
async def approve_deposit(callback: CallbackQuery, bot: Bot, did: int):
    ...
    if method == "stars":
        ...
        gross, tax, net, dep_tax_pct = 0, 0, 0, 0
    ...
    try:
        await callback.message.edit_caption(
            ...
            f"🪙 Tokens: {net:,.4f} (Tax: {dep_tax_pct}%)" + promo_msg,
            parse_mode="HTML"
        )
    except:
        try:
            await callback.message.edit_text(
                f"✅ <b>DEPOSIT APPROVED</b> #{did}\n🪙 {net:,.4f} Tokens" + promo_msg,
                parse_mode="HTML"
            )
        except:
            pass
    ...
    try:
        await bot.send_message(
            ...
            success_text(
                ...
                f"✅ Credited: <b>{net:,.4f} Tokens</b>" + promo_msg
            ),
            parse_mode="HTML"
        )
    except Exception as e:
