...
async def pvp_bet_input(message: Message, state: FSMContext):
    ...
    duel_id = create_duel(user_id, message.from_user.first_name or str(user_id), game_type, bet, house_fee, chat_id)
    await db.update_token_balance(user_id, -bet)
    game_emoji = GAME_EMOJIS.get(game_type, "⚔️")
    ...
    await state.clear()
    # If in private, also notify group if set
        ...
        if group_id:
            try:
                ...
                notify = (
                    ...
                    f"Player: <b>{message.from_user.first_name}</b>\n"
                    f"💵 Bet: <b>{bet:,.4f} Tokens</b>"
                )
                await bot.send_message(int(group_id), notify, parse_mode="HTML", reply_markup=gb.as_markup())
            except:
                pass
...
async def cb_pvp_join(callback: CallbackQuery):
    ...
    if game_type == "dice":
        loading_msg_text = (
            ...
            f"🎲 <b>{duel['creator_name']}</b> vs <b>{opponent_name}</b>\n"
            f"💵 Bet: <b>{duel['bet']:,.4f} T</b>\n{SEP}\n"
            f"⏳ Starting in 60 seconds..."
        )
        target_chat = group_chat or callback.message.chat.id
        try:
            bar_msg = await bot.send_message(target_chat, loading_msg_text, parse_mode="HTML")
        except:
            bar_msg = await callback.message.answer(loading_msg_text, parse_mode="HTML")
        await callback.answer("You joined! Get ready to roll 🎲")
        asyncio.create_task(_pvp_dice_countdown(
            duel_id, creator_id, user_id,
            bar_msg.chat.id, bar_msg.message_id,
            duel["creator_name"], opponent_name, duel["bet"]
        ))
    else:
        resolved = resolve_duel(duel_id)
        if not resolved:
            await callback.answer("Error.", show_alert=True); return
        await _finish_pvp(resolved, callback.message, creator_id, user_id, group_chat)
        await callback.answer()


async def _pvp_dice_countdown(duel_id: str, creator_id: int, opponent_id: int,
                               chat_id: int, msg_id: int,
                               creator_name: str, opponent_name: str, bet: float):
    """60-second countdown bar in group, then ask both to roll."""
    ...
    roll_text = (
        f"🎲 <b>ROLL NOW!</b>\n{SEP}\n"
        f"<b>{creator_name}</b> vs <b>{opponent_name}</b>\n"
        f"Both players: send 🎲 in <b>30 seconds</b>!\n"
        f"👇 Tap to copy dice emoji:\n<code>🎲</code>"
    )
    try:
        await bot.edit_message_text(roll_text, chat_id=chat_id, message_id=msg_id, parse_mode="HTML")
    except:
        pass
    # Notify both privately too
    for uid, name in [(creator_id, creator_name), (opponent_id, opponent_name)]:
        try:
            await bot.send_message(uid,
                f"🎲 <b>ROLL NOW!</b> Send dice emoji in group or here:\n<code>🎲</code>",
                parse_mode="HTML"
            )
        except:
            pass
    # 30-sec timeout then auto-resolve
    asyncio.create_task(_pvp_roll_timeout(duel_id, creator_id, opponent_id, chat_id))
...
async def _pvp_roll_timeout(duel_id: str, creator_id: int, opponent_id: int, timeout: int):
    await asyncio.sleep(timeout)
    duel = active_duels.get(duel_id)
    if not duel or duel["status"] != "active":
        return
    # Force random rolls for anyone who didn't roll
    ...
    if resolved:
        text = duel_result_text(resolved) + "\n\n⏰ <i>Time expired — auto-resolved</i>"
        resolved["net_prize"] = resolved.get("net_prize", duel["bet"] * 2)
        await _finish_pvp_by_ids(resolved, creator_id, opponent_id, text)
...
async def cb_pvp_list(callback: CallbackQuery):
    ...
    for d in duels[:5]:
        ...
        builder.button(text=f"⚔️ Join {d['creator_name']} ({d['bet']:,.0f}T)", callback_data=f"pvp_join_{d['id']}")
    builder.adjust(1)
...
async def cmd_duel_group(message: Message, state: FSMContext):
    """Create PVP duel from group or private — /duel dice 500"""
    parts = message.text.split()
    game_type = "dice"
    bet = None
    if len(parts) >= 2:
        if parts[1].lower() in ("dice", "coinflip", "highroll", "cf", "hr"):
            mapping = {"dice": "dice", "coinflip": "coinflip", "cf": "coinflip",
                      "highroll": "highroll", "hr": "highroll"}
            game_type = mapping.get(parts[1].lower(), "dice")
        else:
            try: bet = float(parts[1].replace(",", ""))
            except: pass
    if len(parts) >= 3:
        try: bet = float(parts[2].replace(",", ""))
        except: pass
    if bet is None:
        await message.reply(
            f"Usage: <code>/duel dice 500</code>\n"
            f"Games: <code>dice</code> | <code>coinflip</code> | <code>highroll</code>",
            parse_mode="HTML"
        ); return
    user_id = message.from_user.id
    user = await db.get_user(user_id)
    if not user:
        await message.reply("Please /start the bot first!"); return
    if user["token_balance"] < bet:
        await message.reply(error_text(f"❌ Need {bet:,.0f} Tokens. You have {user['token_balance']:,.4f}."), parse_mode="HTML"); return
    house_fee = float(await db.get_setting("pvp_house_fee") or "5")
    chat_id = message.chat.id
    duel_id = create_duel(user_id, message.from_user.first_name or str(user_id), game_type, bet, house_fee, chat_id)
    await db.update_token_balance(user_id, -bet)
    ...
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    builder = InlineKeyboardBuilder()
    builder.button(text=f"⚔️ JOIN — {bet:,.0f} T", callback_data=f"pvp_join_{duel_id}")
...
async def cmd_joinduel(message: Message):
    ...
    game_type = duel["game_type"]
    creator_id = duel["creator_id"]
    opponent_name = message.from_user.first_name or str(user_id)
    if game_type == "dice":
        roll_text = (
            f"⚔️ <b>DUEL STARTED!</b>\n{SEP}\n"
            f"🎲 Send dice emoji in <b>30 seconds</b>!\n"
            f"👇 Tap to copy:\n<code>🎲</code>"
        )
        await message.reply(roll_text, parse_mode="HTML")
        try:
            await bot.send_message(creator_id, roll_text, parse_mode="HTML")
        except:
            pass
        asyncio.create_task(_pvp_roll_timeout(duel_id, creator_id, user_id, 30))
    else:
        resolved = resolve_duel(duel_id)
        if resolved:
            await _finish_pvp_by_ids(resolved, creator_id, user_id)
# ═══════════════════════════════════════════════════════════════════════════════
# ─── RAIN ─────────────────────────────────────────────────────────────────────
# ═══════════════════════════════════════════════════════════════════════════════
...
async def cb_admin_rain(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "🌧️ <b>SEND RAIN</b>\n"
        + SEP + "\n"
        "Send: <code>amount winners duration_seconds</code>\n"
        "Example: <code>5000 20 120</code>\n"
        "(5000 tokens, 20 winners, 120 seconds)",
...
async def rain_amount_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply(
            error_text("❌ Format: <code>amount winners duration</code>\nExample: <code>5000 20 120</code>"),
            parse_mode="HTML"
        ); return
    await state.clear()
    ...
    if group_id:
        ...
        await message.reply(
            text + "\n\n⚠️ No group set! Go Admin → New Features → Rain Group ID",
...
async def cb_loser_board(callback: CallbackQuery):
    ...
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    builder = InlineKeyboardBuilder()
    builder.button(text="🔄 Refresh", callback_data="menu_loser_board")
    builder.button(text="🔙 Back", callback_data="menu_leaderboard")
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=builder.as_markup())
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup())
...
async def cb_admin_missions_panel(callback: CallbackQuery):
    ...
    from aiogram.types import InlineKeyboardButton
    builder = InlineKeyboardBuilder()
    text_lines = [f"📋 <b>MISSION ADMIN</b>\n{SEP}"]
    for m in MISSIONS:
        val = await db.get_setting(m["setting_key"])
        status = "🟢" if val != "0" else "🔴"
        text_lines.append(f"{status} {m['icon']} {m['name']} ({m['type']})")
        builder.button(
            text=f"{status} {m['name']}",
            callback_data=f"mission_toggle_{m['id']}"
        )
    builder.adjust(2)
