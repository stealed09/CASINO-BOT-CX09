...
async def resolve_dice_crazy(user_id: int, user_val: int, bot: Bot):
    ...
    if user_val > bot_val:
        ...
        result = (
            f"🎲 <b>DICE Crazy</b>\n{SEP}\n"
            f"🤖 Bot: <b>{bot_val}</b>  |  You: <b>{user_val}</b>\n"
            f"🎉 <b>YOU WIN!</b>\n"
            f"Gross: {gross:,.4f} | Tax: -{tax:,.4f}\n"
            f"✅ +<b>{reward:,.4f} Tokens</b>"
        )
    elif user_val == bot_val:
        # TIE — full refund, no tax
        await db.add_transaction(user_id, "refund", bet, currency="TOKEN")
        result = (
            f"🎲 <b>DICE Crazy</b>\n{SEP}\n"
            f"🤖 Bot: <b>{bot_val}</b>  |  You: <b>{user_val}</b>\n"
            f"🤝 <b>TIE! Bet Refunded.</b>\n"
            f"↩️ <b>{bet:,.4f} Tokens</b> returned"
        )
    else:
        # LOSE
        await db.update_token_balance(user_id, -bet)
    ...
    logger.info(f"Dice Crazy | user={user_id} | bot={bot_val} | user_val={user_val}")
    return True
