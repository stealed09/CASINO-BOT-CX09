...
async def start_mines(callback: CallbackQuery, bot: Bot, bet: float, mines_count: int):
    ...
    # Admin-controlled win percent — affects mine placement bias
    win_percent = float(await db.get_setting("mines_win_percent") or "40")
    # Lower win_percent = mines clustered near tiles users typically click first (edges/corners)
    # Higher win_percent = mines more spread/random
    all_tiles = list(range(GRID_SIZE))
    # Tiles users click first: center and nearby (indices 6,7,8,11,12,13,16,17,18)
    hot_tiles = [6,7,8,11,12,13,16,17,18]
    cold_tiles = [t for t in all_tiles if t not in hot_tiles]
    if win_percent < 40:
        # More mines on hot tiles — user more likely to lose
        hot_mines = min(mines_count, len(hot_tiles))
        cold_mines = mines_count - hot_mines
        mine_positions = random.sample(hot_tiles, hot_mines) + random.sample(cold_tiles, min(cold_mines, len(cold_tiles)))
    else:
        # Fair random placement
