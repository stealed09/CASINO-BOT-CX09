...
def get_multiplier(reels: List[str], win_percent: float = 45.0) -> Tuple[float, str]:
    """Return (multiplier, result_label). 0 = loss. win_percent controls house edge."""
    # House edge check — force loss based on win_percent
    import random as _r
    if _r.random() * 100 > win_percent:
        return 0.0, "❌ No match"
