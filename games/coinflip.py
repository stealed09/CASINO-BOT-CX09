...
# Keep for backward compat import
def coinflip_choice_kb(amount: str) -> InlineKeyboardMarkup:
    return coinflip_pick_kb(float(amount))
...
async def coinflip_side_selected(callback: CallbackQuery, bot: Bot, bet: float, choice: str):
    """Bot tosses coin — no animation, just result."""
    ...
    try:
        ...
        result_label = "Heads 👑" if result == "heads" else "Tails 🦅"
