...
def resolve_duel(duel_id: str, creator_roll: int = None, opponent_roll: int = None) -> Optional[dict]:
    ...
    if game == "dice":
        # Use provided rolls (from Telegram dice) or fallback to random
        c_roll = creator_roll if creator_roll else random.randint(1, 6)
        o_roll = opponent_roll if opponent_roll else random.randint(1, 6)
        while c_roll == o_roll:
            o_roll = random.randint(1, 6)
        winner = duel["creator_id"] if c_roll > o_roll else duel["opponent_id"]
        duel["result"] = {"creator_roll": c_roll, "opponent_roll": o_roll}
        ...
        c_roll = creator_roll if creator_roll else random.randint(1, 100)
        o_roll = opponent_roll if opponent_roll else random.randint(1, 100)
        while c_roll == o_roll:
    ...
    # Store pending dice rolls state
    duel["pending_creator_roll"] = None
    duel["pending_opponent_roll"] = None
...
def set_dice_roll(duel_id: str, user_id: int, roll: int) -> Optional[dict]:
    """Store a dice roll for a user in a duel. Returns duel if both rolled."""
    duel = active_duels.get(duel_id)
    if not duel or duel["status"] != "active":
        return None
    if duel["creator_id"] == user_id:
        duel["pending_creator_roll"] = roll
    elif duel["opponent_id"] == user_id:
        duel["pending_opponent_roll"] = roll
    # Check if both rolled
    if duel.get("pending_creator_roll") and duel.get("pending_opponent_roll"):
        return duel
    return None


def get_pending_duel_for_user(user_id: int) -> Optional[dict]:
    """Find active duel waiting for this user's dice roll."""
    for duel in active_duels.values():
        if duel["status"] != "active":
            continue
        if duel["game_type"] != "dice":
            continue
        if duel["creator_id"] == user_id and not duel.get("pending_creator_roll"):
            return duel
        if duel["opponent_id"] == user_id and not duel.get("pending_opponent_roll"):
            return duel
    return None
