...
from ui.keyboards import (
    ...
    admin_lootbox_cases_kb, admin_lootbox_case_edit_kb,
    admin_vip_levels_kb, admin_vip_level_edit_kb,
    mines_count_kb, mines_grid_kb,
...
from missions import MISSIONS, MISSION_BY_ID, missions_text, claimable_missions, get_period_key
from loser_board import generate_loser_board, loser_board_text
...
from games.mines import prompt_mines, start_mines, reveal_tile, cashout_mines
from promo import create_promo, get_promo, get_all_promos, validate_promo, use_promo, deactivate_promo, promo_info_text
from games.coinflip import prompt_coinflip, coinflip_side_selected
...
class PromoFSM(StatesGroup):
    waiting_code = State()
    waiting_bonus_pct = State()
    waiting_min_dep = State()
    waiting_max_dep = State()
    waiting_hours = State()
...
SETTING_PROMPTS = {
    ...
    "aset_crash_house_edge":    ("crash_house_edge", "Enter Crash house edge % (e.g. 4 = 4% house edge, higher = more crashes early):"),
    "aset_crash_win_tax":       ("crash_win_tax", "Enter Crash win tax % (e.g. 5 = 5% deducted from Crash wins only):"),
    "aset_mines_min":           ("mines_min_count", "Enter minimum mines count (default 5, min 5):"),
    "aset_mines_max":           ("mines_max_count", "Enter maximum mines count (default 23):"),
    "aset_mines_edge":          ("mines_house_edge", "Enter Mines house edge % (higher = less payout, default 5):"),
    "aset_mines_winpct":        ("mines_win_percent", "Enter Mines win percent % (e.g. 40 = user wins 40% of the time, default 40):"),
    "aset_slots_win_pct":       ("slots_win_percent", "Enter Slots win chance % (e.g. 40 = 40% chance to win something):"),
    ...
    "aset_loser_board_count":    ("loser_board_count", "Enter loser board entries count (e.g. 3 or 4):"),
    "aset_loser_board_max_amount": ("loser_board_max_amount", "Enter max fake loss amount in Tokens (e.g. 50000):"),
}
...
async def msg_upi_amount(message: Message, state: FSMContext):
    ...
    await state.set_state(DepositFSM.upi_promo)
    from aiogram.utils.keyboard import InlineKeyboardBuilder as IKB
    from aiogram.types import InlineKeyboardButton as IKBtn
    builder = IKB()
    builder.row(IKBtn(text="⏭ Skip Promo", callback_data="upi_promo_skip"))
    await message.answer(
        "🎁 <b>Have a promo code?</b>\n\nSend your promo code or press Skip:",
        parse_mode="HTML", reply_markup=builder.as_markup()
    )


@dp.message(DepositFSM.upi_promo)
async def msg_upi_promo(message: Message, state: FSMContext):
    code = (message.text or "").strip().upper()
    data = await state.get_data()
    amount = data.get("upi_inr_amount", 0)
    if code:
        await db.set_setting(f"pending_promo_{message.from_user.id}", code)
    await state.set_state(DepositFSM.upi_screenshot)
    await start_upi_deposit(message, bot, amount)


@dp.callback_query(F.data == "upi_promo_skip")
async def cb_upi_promo_skip(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    data = await state.get_data()
    amount = data.get("upi_inr_amount", 0)
    await state.set_state(DepositFSM.upi_screenshot)
    # Pass callback.message - it has answer_photo method
    # But from_user will be None on bot messages, so patch it
    class FakeMsg:
        ...
        async def answer_photo(self, **kwargs):
            await bot.send_photo(chat_id=self.chat.id, **kwargs)
        async def answer(self, text, **kwargs):
            await bot.send_message(chat_id=self.chat.id, text=text, **kwargs)
    await start_upi_deposit(FakeMsg(callback), bot, amount)


@dp.callback_query(F.data == "crypto_promo_skip")
async def cb_crypto_promo_skip(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    data = await state.get_data()
    token_amount = data.get("crypto_token_amount", 0)
    await state.clear()
    rate = await db.get_setting("usd_to_token_rate") or "85"
    dep_tax = await db.get_effective_deposit_tax(callback.from_user.id)
    usd_amount = round(token_amount / float(rate), 4)
    try:
        await callback.message.edit_text(
            "₿ <b>CRYPTO DEPOSIT</b>\n" + SEP + "\n"
            f"🪙 Tokens to buy: <b>{token_amount:,.0f}</b>\n"
            f"📊 Rate: $1 = <b>{rate} Tokens</b>\n"
            f"💵 USD: <b>${usd_amount:.2f}</b> | Tax: <b>{dep_tax}%</b>\n\n"
...
async def cb_upi_done(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(DepositFSM.upi_screenshot)
    # Always send new message — edit_caption on photo is unreliable
    await callback.message.answer(
...
async def msg_nowpay_token_amount(message: Message, state: FSMContext):
    ...
    await state.update_data(crypto_token_amount=amount)
    await state.set_state(DepositFSM.crypto_promo)
    from aiogram.utils.keyboard import InlineKeyboardBuilder as IKB
    from aiogram.types import InlineKeyboardButton as IKBtn
    builder = IKB()
    builder.row(IKBtn(text="⏭ Skip Promo", callback_data="crypto_promo_skip"))
    await message.answer(
        "🎁 <b>Have a promo code?</b>\n\nSend your promo code or press Skip:",
        parse_mode="HTML", reply_markup=builder.as_markup()
    )
...
@dp.message(DepositFSM.crypto_promo)
async def msg_crypto_promo(message: Message, state: FSMContext):
    code = (message.text or "").strip().upper()
    data = await state.get_data()
    token_amount = data.get("crypto_token_amount", 0)
    if code:
...
async def cmd_cf(message: Message, state: FSMContext):
    ...
    try:
        await coinflip_side_selected(callback, bot, amount, choice)
...
async def cb_crash_start(callback: CallbackQuery, state: FSMContext):
    ...
    await db.increment_mission_field(user_id, __import__("datetime").date.today().isoformat(), "wager", bet)
    await db.increment_mission_field(user_id, __import__("datetime").date.today().isoformat(), "games_played")
    await db.increment_mission_field(user_id, __import__("datetime").date.today().isoformat(), "crash_plays")
    ...
    _crash_tax = await db.get_setting("crash_win_tax")
    game_tax = float(_crash_tax if _crash_tax else await db.get_setting("game_tax_percent") or "5")
    crash_house_edge = float(await db.get_setting("crash_house_edge") or "4")
    from games.crash import crash_message
    from aiogram.utils.keyboard import InlineKeyboardBuilder as _IKB
    _ckb = _IKB()
    _ckb.button(text="💸 CASHOUT NOW", callback_data="crash_cashout")
    msg = await callback.message.answer(
        crash_message(bet, 1.00, auto_cashout, server_seed_hash=hash_server_seed(ss)),
        parse_mode="HTML",
        reply_markup=_ckb.as_markup()
    )
    await state.clear()
    asyncio.create_task(run_crash_game(
        ...
        house_edge_pct=crash_house_edge
    ))
    await callback.answer()
...
async def slots_bet_input(message: Message, state: FSMContext):
    ...
    await db.increment_mission_field(user_id, __import__("datetime").date.today().isoformat(), "games_played")
    # Spinning animation
    spin_msg = await message.reply("🎰 Spinning...", parse_mode="HTML")
    frames = [
        "🎰 | ❓ | ❓ | ❓ |",
        "🎰 | 🍒 | ❓ | ❓ |",
        "🎰 | 🍒 | 🍋 | ❓ |",
    ...
    slots_win_pct = float(await db.get_setting("slots_win_percent") or "45")
    multiplier, label = get_multiplier(reels, slots_win_pct)
...
async def cb_game_coinflip(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    user = await db.get_user(callback.from_user.id)
    if not user:
        await callback.message.answer("Use /start first!"); return
    await state.update_data(game_key="coinflip")
...
async def cb_game_mines(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    user = await db.get_user(callback.from_user.id)
    if not user:
        await callback.message.answer("Use /start first!"); return
    min_bet = await db.get_setting("mines_min_bet") or "10"
    await state.update_data(game_key="mines")
    await state.set_state(GameBetFSM.waiting_bet)
    try:
        await callback.message.edit_text(
            f"💣 <b>MINES</b>\n{SEP}\n"
            f"💵 Min bet: <b>{min_bet}</b> Tokens\n\n"
            f"Send your bet amount:",
            parse_mode="HTML", reply_markup=back_kb("menu_games")
        )
    except:
...
async def cb_game(callback: CallbackQuery, state: FSMContext):
    ...
    if cmd in ("crash", "slots", "mines", "coinflip"):
        return
...
async def msg_game_bet(message: Message, state: FSMContext):
    ...
    # Coinflip — uses callback-based flow
    if game_key == "coinflip":
        await prompt_coinflip(message, amount)
        return
    # Mines — uses callback-based flow
    if game_key == "mines":
        await prompt_mines(message, amount)
        return
...
async def cb_admin_set(callback: CallbackQuery, state: FSMContext):
    ...
    DIRECT_TOGGLES = {
        ...
        "aset_loser_board_enabled": "loser_board_enabled",
    }
...
async def msg_admin_setting(message: Message, state: FSMContext):
    ...
    # ── Admin Lootbox input ────────────────────────────────────────────
    if data.get("admin_lb_key"):
        lb_key = data["admin_lb_key"]
        lb_field = data.get("admin_lb_field")
        from lootbox import get_cases, save_cases
        cases = await get_cases()
        case = cases.get(lb_key)
        if case:
            if lb_field == "price":
                try:
                    case["price"] = int(float(message.text.strip()))
                    cases[lb_key] = case
                    await save_cases(cases)
                    await message.answer(f"✅ <b>{lb_key}</b> price updated to <b>{case['price']:,} T</b>",
                        parse_mode="HTML", reply_markup=back_kb("admin_lootbox_panel"))
                except:
                    await message.answer("❌ Invalid price. Send a number.", parse_mode="HTML")
            elif lb_field == "reward":
                idx = data.get("admin_lb_reward_idx", 0)
                try:
                    parts = message.text.strip().split("|")
                    case["rewards"][idx] = {"type": "tokens", "amount": float(parts[1].strip()),
                        "label": parts[0].strip(), "weight": float(parts[2].strip())}
                    cases[lb_key] = case
                    await save_cases(cases)
                    await message.answer(f"✅ Reward #{idx+1} in <b>{lb_key}</b> updated!",
                        parse_mode="HTML", reply_markup=back_kb("admin_lootbox_panel"))
                except:
                    await message.answer("❌ Format: <code>label|amount|weight</code>", parse_mode="HTML")
        await state.clear()
        return

    # ── Admin VIP input ────────────────────────────────────────────────
    if data.get("admin_vip_field") is not None:
        vip_field = data["admin_vip_field"]
        vip_level = data["admin_vip_level"]
        try:
            val = float(message.text.strip())
            from vip import get_vip_levels, save_vip_levels
            levels = await get_vip_levels()
            for lvl in levels:
                if lvl["level"] == vip_level:
                    lvl[vip_field] = val
            await save_vip_levels(levels)
            await message.answer(f"✅ Level {vip_level} <b>{vip_field}</b> → <b>{val}</b>",
                parse_mode="HTML", reply_markup=back_kb("admin_vip_panel"))
        except:
            await message.answer("❌ Invalid value. Send a number.", parse_mode="HTML")
        await state.clear()
        return
...
async def on_dice_message(message: Message):
    """Handle physical dice — resolves crazy mode OR pvp duel."""
    user_id = message.from_user.id
    emoji = message.dice.emoji
    roll_value = message.dice.value

    # Check PVP duel first (works in group AND private)
    # emoji mapping: 🎲=dice, 🪙=coinflip, 🎯=highroll
    EMOJI_TO_GAME = {"🎲": "dice", "🎯": "highroll"}
    pvp_game = EMOJI_TO_GAME.get(emoji)
    if pvp_game:
        duel = get_pending_duel_for_user(user_id)
        if duel and duel["game_type"] == pvp_game:
            duel_id = duel["id"]
            group_chat = duel.get("chat_id")
            await asyncio.sleep(4)  # Wait for Telegram dice animation to complete
            # Re-read value after animation (guaranteed final value)
            roll_value = message.dice.value
            result = set_dice_roll(duel_id, user_id, roll_value)
            rolled_name = message.from_user.first_name or str(user_id)
            other_name = duel["opponent_name"] if user_id == duel["creator_id"] else duel["creator_name"]
            if result is None:
                announce = f"{emoji} <b>{rolled_name}</b> rolled: <b>{roll_value}</b>\nWaiting for <b>{other_name}</b>..."
                target = group_chat or message.chat.id
                try:
                    await bot.send_message(target, announce, parse_mode="HTML")
                except:
                    await message.reply(announce, parse_mode="HTML")
            else:
                resolved = resolve_duel(duel_id, result["creator_roll"], result["opponent_roll"])
                if resolved:
                    text = duel_result_text(resolved)
                    await _finish_pvp_by_ids(
                        resolved, duel["creator_id"], duel["opponent_id"], text,
                        group_chat_id=group_chat or message.chat.id
                    )
            return  # Don't fall through to crazy mode
    # Crazy mode check — only dice emoji
...
async def main():
    ...
    group_commands = [
        ...
        BotCommand(command="duel",        description="Create PVP duel ⚔️ (e.g. /duel dice 500)"),
...
async def cb_missions(callback: CallbackQuery):
    ...
    user_id = callback.from_user.id
    prog = await db.get_all_mission_progress(user_id)
    claimed = prog.get("claimed_missions", [])
    enabled_ids = await db.get_enabled_missions()
    text = missions_text(prog, claimed, enabled_ids)
    claimable = claimable_missions(prog, claimed, enabled_ids)
...
async def cb_mission_claim(callback: CallbackQuery):
    ...
    from missions import is_complete, get_mission_progress_value
    enabled_ids = await db.get_enabled_missions()
    if mission_id not in enabled_ids:
        await callback.answer("This mission is disabled.", show_alert=True); return
    prog_val = get_mission_progress_value(mission, prog)
    if prog_val < mission["target"]:
        await callback.answer(
            f"Not complete yet! {prog_val}/{mission['target']} {mission['desc']}",
            show_alert=True
        ); return
    period_key = get_period_key(mission["type"])
    await db.add_mission_claim(user_id, period_key, mission_id)
...
async def cb_lootbox_menu(callback: CallbackQuery):
    ...
    from lootbox import get_cases
    await get_cases()  # refresh cache
...
async def cb_pvp_create(callback: CallbackQuery, state: FSMContext):
    ...
    game_emoji = GAME_EMOJIS.get(game_type, "⚔️")
    await state.update_data(pvp_game_type=game_type, pvp_chat_id=callback.message.chat.id)
    await state.set_state(PvpFSM.waiting_bet)
    await callback.message.answer(
        f"{game_emoji} <b>{game_name}</b>\n{SEP}\n"
        f"Send your bet amount in Tokens:\n"
        f"<i>Example: 500</i>",
...
async def pvp_bet_input(message: Message, state: FSMContext):
    ...
    origin_chat = data.get("pvp_chat_id", message.chat.id)
    try:
        bet = float(message.text.strip().replace(",", ""))
        if bet <= 0: raise ValueError
    except:
    ...
    user = await db.get_user(user_id)
    if not user:
        await message.reply(error_text("❌ Please /start the bot first!"), parse_mode="HTML"); return
    if user["token_balance"] < bet:
        await message.reply(
            error_text(f"❌ Not enough balance! You have {user['token_balance']:,.4f} Tokens."),
            parse_mode="HTML"
        ); return
    house_fee = float(await db.get_setting("pvp_house_fee") or "5")
    player_name = message.from_user.first_name or str(user_id)
    duel_id = create_duel(user_id, player_name, game_type, bet, house_fee, origin_chat)
    await db.update_token_balance(user_id, -bet)
    await state.clear()
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    builder = InlineKeyboardBuilder()
    builder.button(text=f"⚔️ JOIN DUEL — {bet:,.0f} T", callback_data=f"pvp_join_{duel_id}")
    text = duel_waiting_text(active_duels[duel_id])
    sent = await message.reply(text, parse_mode="HTML", reply_markup=builder.as_markup())
    # Always notify group
    group_id = await db.get_setting("pvp_group_id") or await db.get_setting("rain_group_id")
    if group_id and str(message.chat.id) != str(group_id):
        try:
            from aiogram.utils.keyboard import InlineKeyboardBuilder as IKB
            gb = IKB()
            gb.button(text=f"⚔️ JOIN — {bet:,.0f} T", callback_data=f"pvp_join_{duel_id}")
            await bot.send_message(int(group_id),
                f"⚔️ <b>NEW DUEL!</b>\n{SEP}\n"
                f"Game: <b>{GAME_NAMES.get(game_type)}</b>\n"
                f"Player: <b>{player_name}</b>\n"
                f"💵 Bet: <b>{bet:,.4f} Tokens</b>",
                parse_mode="HTML", reply_markup=gb.as_markup()
            )
        except:
            pass
...
async def cb_pvp_join(callback: CallbackQuery):
    ...
    opponent_name = callback.from_user.first_name or str(user_id)
    group_chat = duel.get("chat_id") or callback.message.chat.id
    creator_name = duel["creator_name"]
    bet = duel["bet"]

    if game_type == "dice":
        # Ask IMMEDIATELY — no countdown
        roll_text = (
            f"⚔️ <b>DUEL ON!</b>\n{SEP}\n"
            f"🎲 <b>{creator_name}</b> vs <b>{opponent_name}</b>\n"
            f"💵 Bet: <b>{bet:,.4f} Tokens</b>\n{SEP}\n"
            f"Both players: <b>send 🎲 dice emoji NOW in this chat!</b>\n"
            f"⏳ You have <b>90 seconds</b>\n"
            f"👇 Tap to copy:\n<code>🎲</code>"
        )
        try:
            await callback.message.edit_text(roll_text, parse_mode="HTML")
        except:
            await callback.message.answer(roll_text, parse_mode="HTML")
        if group_chat and group_chat != callback.message.chat.id:
            try:
                await bot.send_message(group_chat, roll_text, parse_mode="HTML")
            except:
                pass
        # Notify creator privately
        try:
            await bot.send_message(creator_id,
                f"⚔️ <b>{opponent_name}</b> joined your duel!\n"
                f"🎲 Send dice emoji in the group NOW!\n<code>🎲</code>",
                parse_mode="HTML"
            )
        except:
            pass
        asyncio.create_task(_pvp_roll_timeout(duel_id, creator_id, user_id, group_chat, 90))

    elif game_type == "mines_placeholder":
        # Use Telegram coin emoji
        coin_text = (
            f"⚔️ <b>COINFLIP DUEL!</b>\n{SEP}\n"
            f"🪙 <b>{creator_name}</b> vs <b>{opponent_name}</b>\n"
            f"💵 Bet: <b>{bet:,.4f} Tokens</b>\n{SEP}\n"
            f"Both players: <b>send 🪙 coin emoji NOW!</b>\n"
            f"👇 Tap to copy:\n<code>🪙</code>"
        )
        try:
            await callback.message.edit_text(coin_text, parse_mode="HTML")
        except:
            await callback.message.answer(coin_text, parse_mode="HTML")
        if group_chat and group_chat != callback.message.chat.id:
            try:
                await bot.send_message(group_chat, coin_text, parse_mode="HTML")
            except:
                pass
        try:
            await bot.send_message(creator_id,
                f"⚔️ <b>{opponent_name}</b> joined! Send 🪙 coin in group NOW!\n<code>🪙</code>",
                parse_mode="HTML"
            )
        except:
            pass
    ...
    elif game_type == "highroll":
        # Use dart emoji
        dart_text = (
            f"⚔️ <b>HIGH ROLL DUEL!</b>\n{SEP}\n"
            f"🎯 <b>{creator_name}</b> vs <b>{opponent_name}</b>\n"
            f"💵 Bet: <b>{bet:,.4f} Tokens</b>\n{SEP}\n"
            f"Both players: <b>send 🎯 dart emoji NOW!</b>\n"
            f"👇 Tap to copy:\n<code>🎯</code>"
        )
        try:
            await callback.message.edit_text(dart_text, parse_mode="HTML")
        except:
            await callback.message.answer(dart_text, parse_mode="HTML")
        if group_chat and group_chat != callback.message.chat.id:
            try:
                await bot.send_message(group_chat, dart_text, parse_mode="HTML")
            except:
                pass
        try:
            await bot.send_message(creator_id,
                f"⚔️ <b>{opponent_name}</b> joined! Send 🎯 dart in group NOW!\n<code>🎯</code>",
                parse_mode="HTML"
            )
        except:
            pass
...
async def _pvp_roll_timeout(duel_id: str, creator_id: int, opponent_id: int,
                             group_chat: int, timeout: int = 30):
    """Auto-resolve if someone doesn't roll in time."""
    await asyncio.sleep(timeout)
    duel = active_duels.get(duel_id)
    if not duel or duel["status"] != "active":
        return
    c_roll = duel.get("creator_roll") or random.randint(1, 6)
    o_roll = duel.get("opponent_roll") or random.randint(1, 6)
    while c_roll == o_roll:
        o_roll = random.randint(1, 6)
    resolved = resolve_duel(duel_id, c_roll, o_roll)
    if resolved:
        text = duel_result_text(resolved) + "\n\n⏰ <i>Auto-resolved (time expired)</i>"
        resolved["net_prize"] = resolved.get("net_prize", duel["bet"] * 2)
        await _finish_pvp_by_ids(resolved, creator_id, opponent_id, text, group_chat_id=group_chat)


async def _finish_pvp(resolved: dict, message, creator_id: int, opponent_id: int,
                       group_chat: int = None):
    winner_id = resolved["winner_id"]
    net_prize = resolved.get("net_prize", resolved.get("bet", 0) * 2)
    await db.update_token_balance(winner_id, net_prize)
    await db.add_transaction(winner_id, "pvp_win", net_prize, "completed", "TOKEN")
    loser_id = creator_id if winner_id == opponent_id else opponent_id
    await db.add_transaction(loser_id, "pvp_loss", resolved["bet"], "completed", "TOKEN")
    text = duel_result_text(resolved)
    for uid in [creator_id, opponent_id]:
        try:
            await bot.send_message(uid, text, parse_mode="HTML", reply_markup=back_to_main_kb())
        except:
            pass
    gid = group_chat
    if not gid:
        gid_str = await db.get_setting("pvp_group_id") or await db.get_setting("rain_group_id")
        if gid_str:
            try: gid = int(gid_str)
            except: pass
    if gid:
...
async def _finish_pvp_by_ids(resolved: dict, creator_id: int, opponent_id: int,
                              text: str = None, group_chat_id: int = None):
    if not text:
        text = duel_result_text(resolved)
    winner_id = resolved["winner_id"]
    net_prize = resolved.get("net_prize", resolved.get("bet", 0) * 2)
...
async def cb_admin_rain(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"🌧️ <b>SEND RAIN</b>\n{SEP}\n"
        f"Send: <code>amount winners seconds</code>\n"
        f"Example: <code>5000 20 120</code>\n"
        f"(5000 tokens, 20 winners, 120 seconds duration)",
...
async def rain_amount_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        winners = int(parts[1]) if len(parts) > 1 else 20
        duration = int(parts[2]) if len(parts) > 2 else 60
        if amount <= 0 or winners <= 0 or duration <= 0: raise ValueError
    except:
        await message.reply(
            error_text("❌ Format: <code>amount winners seconds</code>\nExample: <code>5000 20 120</code>"),
            parse_mode="HTML"
        ); return
    await state.clear()
    rain = create_rain(amount, winners, duration_seconds=duration, triggered_by="Admin")
    ...
    if not sent_to_group:
        await message.reply(
            text + "\n\n⚠️ <b>No group set!</b> Go Admin → New Features → Rain Group ID",
            parse_mode="HTML", reply_markup=rain_catch_kb()
        )
    await message.reply(
        f"🌧️ <b>Rain Started!</b>\n"
        f"💰 Amount: <b>{amount:,.0f} Tokens</b>\n"
        f"👥 Max Winners: <b>{winners}</b>\n"
        f"⏳ Duration: <b>{duration} seconds</b>",
        parse_mode="HTML"
    )
    asyncio.create_task(_finish_rain_task(amount, winners, duration))
...
async def cb_rain_catch(callback: CallbackQuery):
    ...
    share = round(rain["amount"] / rain["max_winners"], 4)
    await callback.answer(
        f"🌧️ Caught! You are #{count}/{rain['max_winners']}\n"
        f"You will receive ~{share:,.4f} Tokens when rain ends!",
        show_alert=True
    )


async def _finish_rain_task(amount: float, max_winners: int, duration: int = 60):
    ...
    for uid in rain["participants"]:
        try:
            ...
            await bot.send_message(
                uid,
                success_text(f"🌧️ Rain ended! You received <b>+{share:,.4f} Tokens</b>"),
                parse_mode="HTML"
            )
        except:
            pass
    ...
    if group_id:
        try:
            await bot.send_message(int(group_id), rain_result_text(rain), parse_mode="HTML")
        except:
            pass
...
async def cb_admin_new_features(callback: CallbackQuery):
    ...
    async def tog(key):
        ...
        return "🟢 ON" if v != "0" and v is not None else "🔴 OFF"
    ...
    text = (
        ...
        f"💀 Loser Board: {await tog('loser_board_enabled')}\n"
    )
    try:
...
# ─── ADMIN LOOTBOX CONFIG ─────────────────────────────────────────────────────

@dp.callback_query(F.data == "admin_lootbox_panel")
async def cb_admin_lootbox_panel(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    from lootbox import get_cases, admin_lootbox_overview_text
    cases = await get_cases()
    text = admin_lootbox_overview_text(cases)
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=admin_lootbox_cases_kb(cases))
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=admin_lootbox_cases_kb(cases))
    await callback.answer()


@dp.callback_query(F.data.startswith("admin_lb_edit_"))
async def cb_admin_lb_edit(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    case_key = callback.data.replace("admin_lb_edit_", "")
    from lootbox import get_cases, admin_case_detail_text
    cases = await get_cases()
    case = cases.get(case_key)
    if not case:
        await callback.answer("Case not found.", show_alert=True); return
    text = admin_case_detail_text(case_key, case)
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=admin_lootbox_case_edit_kb(case_key, case))
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=admin_lootbox_case_edit_kb(case_key, case))
    await callback.answer()


@dp.callback_query(F.data.startswith("admin_lb_price_"))
async def cb_admin_lb_price(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    case_key = callback.data.replace("admin_lb_price_", "")
    await state.update_data(admin_lb_key=case_key, admin_lb_field="price")
    await state.set_state(AdminFSM.waiting_value)
    await callback.message.answer(
        f"💰 Enter new <b>price</b> (tokens) for <b>{case_key}</b> case:\nExample: <code>500</code>",
        parse_mode="HTML", reply_markup=back_kb("admin_lootbox_panel"))
    await callback.answer()


@dp.callback_query(F.data.startswith("admin_lb_reward_"))
async def cb_admin_lb_reward(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    parts = callback.data.replace("admin_lb_reward_", "").rsplit("_", 1)
    case_key, reward_idx = parts[0], int(parts[1])
    await state.update_data(admin_lb_key=case_key, admin_lb_field="reward", admin_lb_reward_idx=reward_idx)
    await state.set_state(AdminFSM.waiting_value)
    await callback.message.answer(
        f"✏️ Edit reward #{reward_idx+1} for <b>{case_key}</b>\n\n"
        f"Format: <code>label|amount|weight</code>\nExample: <code>500 Tokens|500|30</code>",
        parse_mode="HTML", reply_markup=back_kb("admin_lootbox_panel"))
    await callback.answer()
...
async def cb_admin_vip_panel(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    from vip import get_vip_levels, admin_vip_overview_text
    levels = await get_vip_levels()
    text = admin_vip_overview_text(levels)
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=admin_vip_levels_kb(levels))
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=admin_vip_levels_kb(levels))
    await callback.answer()


@dp.callback_query(F.data.startswith("admin_vip_edit_"))
async def cb_admin_vip_edit(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    level_idx = int(callback.data.replace("admin_vip_edit_", ""))
    from vip import get_vip_levels
    levels = await get_vip_levels()
    lvl = next((l for l in levels if l["level"] == level_idx), None)
    if not lvl:
        await callback.answer("Level not found.", show_alert=True); return
    text = (
        f"{lvl['badge']} <b>{lvl['name']}</b> (Level {lvl['level']})\n{SEP}\n"
        f"Min Wager: <b>{lvl['min_wager']:,.0f}</b>\n"
        f"WD Tax Discount: <b>{lvl['withdraw_tax_discount']}%</b>\n"
        f"Dep Tax Discount: <b>{lvl.get('deposit_tax_discount', 0.0)}%</b>\n"
        f"Rakeback: <b>{lvl['cashback_pct']}%</b>\n"
        f"Bonus Mult: <b>{lvl['weekly_bonus_multiplier']}x</b>"
    )
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=admin_vip_level_edit_kb(level_idx))
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=admin_vip_level_edit_kb(level_idx))
    await callback.answer()


_VIP_FIELDS = {
    "wager":    ("min_wager",               "📊 Min Wager"),
    "wddisc":   ("withdraw_tax_discount",   "💸 WD Tax Discount %"),
    "depdisc":  ("deposit_tax_discount",    "📥 Dep Tax Discount %"),
    "rakeback": ("cashback_pct",            "♻️ Rakeback %"),
    "mult":     ("weekly_bonus_multiplier", "🎁 Bonus Multiplier"),
}

for _fkey, (_db_key, _label) in _VIP_FIELDS.items():
    def _make(fkey=_fkey, db_key=_db_key, label=_label):
        @dp.callback_query(F.data.startswith(f"admin_vip_{fkey}_"))
        async def _handler(callback: CallbackQuery, state: FSMContext, _fk=fkey, _dk=db_key, _lb=label):
            if callback.from_user.id not in ADMIN_IDS:
                await callback.answer("No access.", show_alert=True); return
            level_idx = int(callback.data.replace(f"admin_vip_{_fk}_", ""))
            from vip import get_vip_levels
            levels = await get_vip_levels()
            lvl = next((l for l in levels if l["level"] == level_idx), {})
            cur = lvl.get(_dk, 0)
            await state.update_data(admin_vip_field=_dk, admin_vip_level=level_idx)
            await state.set_state(AdminFSM.waiting_value)
            await callback.message.answer(
                f"✏️ Set <b>{_lb}</b> for Level {level_idx}\nCurrent: <b>{cur}</b>\n\nSend new value:",
                parse_mode="HTML", reply_markup=back_kb("admin_vip_panel"))
            await callback.answer()
    _make()
...
@dp.callback_query(F.data.startswith("cf_pick_"))
async def cb_cf_pick(callback: CallbackQuery):
    parts = callback.data.split("_")
    choice = parts[2]
...
async def cb_mines_start(callback: CallbackQuery):
    # mines_start_BET_MINESCOUNT
    parts = callback.data.split("_")
    bet = float(parts[2])
    mines_count = int(parts[3])
    await start_mines(callback, bot, bet, mines_count)


@dp.callback_query(F.data.startswith("mines_tile_"))
async def cb_mines_tile(callback: CallbackQuery):
    # mines_tile_GAMEID_TILEIDX
    parts = callback.data.split("_")
    game_id = parts[2]
    tile_idx = int(parts[3])
    await reveal_tile(callback, bot, game_id, tile_idx)


@dp.callback_query(F.data.startswith("mines_cashout_"))
async def cb_mines_cashout(callback: CallbackQuery):
    game_id = callback.data.replace("mines_cashout_", "")
    await cashout_mines(callback, bot, game_id)


@dp.callback_query(F.data.startswith("mines_dead_"))
async def cb_mines_dead(callback: CallbackQuery):
    await callback.answer("Game over!", show_alert=False)
...
async def cb_menu_promo(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    from aiogram.utils.keyboard import InlineKeyboardBuilder as IKB
    from aiogram.types import InlineKeyboardButton as IKBtn
    all_promos = await get_all_promos()
    active = [p for p in all_promos.values() if p.get("active") and
              datetime.fromisoformat(p["expires_at"]) > datetime.utcnow() and
              len(p["used_by"]) < p["max_uses"]]
    ...
    if active:
        ...
        hours_left = max(0, int((expires - datetime.utcnow()).total_seconds() // 3600))
        text = (
            "🎁 <b>PROMO BONUS AVAILABLE!</b>\n" + SEP + "\n"
            f"Bonus: <b>+{promo['bonus_pct']}%</b> on your deposit\n"
            f"Min deposit: <b>{promo['min_deposit']:,.2f}</b> Tokens\n"
            f"Max deposit: <b>{promo['max_deposit']:,.2f}</b> Tokens\n"
            f"Min withdrawal: <b>{promo.get('min_withdraw', 0):,.2f}</b> Tokens\n"
            f"Expires in: <b>{hours_left}h</b> | Left: <b>{uses_left}</b>\n\n"
            "Enter promo code to activate:"
        )
        builder.row(IKBtn(text="✍️ Enter Promo Code", callback_data="promo_enter_code"))
    else:
        text = (
            "🎁 <b>PROMO CODE</b>\n" + SEP + "\n"
            "No active promo codes right now.\n"
            "Check back later or enter your code manually:"
        )
        builder.row(IKBtn(text="✍️ Enter Promo Code", callback_data="promo_enter_code"))

    builder.row(IKBtn(text="💳 Deposit Without Promo", callback_data="wallet_deposit"))
    builder.row(IKBtn(text="🔙 Back", callback_data="wallet_deposit"))
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=builder.as_markup())
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup())
...
@dp.callback_query(F.data == "promo_enter_code")
async def cb_promo_enter_code(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(PromoFSM.waiting_code)
    try:
        await callback.message.edit_text(
            "🎁 <b>PROMO CODE</b>\n" + SEP + "\nSend your promo code:",
            parse_mode="HTML", reply_markup=back_kb("menu_promo")
        )
    except:
        await callback.message.answer(
            "Send your promo code:",
...
@dp.message(PromoFSM.waiting_code)
async def msg_promo_code(message: Message, state: FSMContext):
    code = (message.text or "").strip().upper()
    await state.update_data(pending_promo=code)
    await state.clear()
    # Store pending promo in user session — applied on next deposit
    await db.set_setting(f"pending_promo_{message.from_user.id}", code)
    promo = await get_promo(code)
    if not promo or not promo.get("active"):
        await message.answer("❌ Invalid promo code!", parse_mode="HTML", reply_markup=back_kb("wallet_deposit"))
        return
    await message.answer(
        f"✅ Promo code <code>{code}</code> saved!\n"
        f"It will be applied on your next deposit automatically.\n\n"
        f"Bonus: <b>+{promo['bonus_pct']}%</b>\n"
        f"Min deposit: <b>{promo['min_deposit']:,.2f}</b>\n"
        f"Max deposit: <b>{promo['max_deposit']:,.2f}</b>",
        parse_mode="HTML", reply_markup=back_kb("wallet_deposit")
    )
...
@dp.callback_query(F.data == "admin_promo_panel")
async def cb_admin_promo_panel(callback: CallbackQuery, state: FSMContext = None):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    await callback.answer()
    all_promos = await get_all_promos()
    lines = ["🎁 <b>PROMO CODE ADMIN</b>\n"]
    for code, p in list(all_promos.items())[-5:]:
        status = "✅" if p.get("active") else "❌"
        min_wd = p.get("min_withdraw", 0)
        lines.append(f"{status} <code>{code}</code> — {p['bonus_pct']}% | min_wd:{min_wd} | {len(p['used_by'])}/{p['max_uses']} used")
    lines.append("\nCreate a new promo code:")
    from aiogram.utils.keyboard import InlineKeyboardBuilder as IKB
    from aiogram.types import InlineKeyboardButton as IKBtn
    builder = IKB()
    builder.row(IKBtn(text="➕ Create Promo", callback_data="admin_promo_create"))
    for code, p in list(all_promos.items())[-3:]:
        if p.get("active"):
            builder.row(IKBtn(text=f"❌ Deactivate {code}", callback_data=f"admin_promo_deact_{code}"))
    builder.row(IKBtn(text="📢 Broadcast Active Promo", callback_data="admin_promo_broadcast"))
    builder.row(IKBtn(text="🔙 Back", callback_data="admin_new_features"))
    try:
        await callback.message.edit_text("\n".join(lines), parse_mode="HTML", reply_markup=builder.as_markup())
    except:
        await callback.message.answer("\n".join(lines), parse_mode="HTML", reply_markup=builder.as_markup())
...
async def cb_admin_promo_create(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    await callback.answer()
    await state.set_state(PromoFSM.waiting_bonus_pct)
    await callback.message.answer(
        "🎁 <b>Create Promo Code</b>\n\nStep 1/5: Enter <b>bonus %</b>\nExample: <code>30</code> for +30% bonus",
        parse_mode="HTML", reply_markup=back_kb("admin_promo_panel")
    )


@dp.message(PromoFSM.waiting_bonus_pct)
async def msg_promo_bonus(message: Message, state: FSMContext):
    try:
        val = float(message.text.strip())
        await state.update_data(promo_bonus=val)
        await state.set_state(PromoFSM.waiting_min_dep)
        await message.answer(
            f"✅ Bonus: <b>+{val}%</b>\n\nStep 2/5: Enter <b>minimum deposit</b> amount\nExample: <code>100</code>",
            parse_mode="HTML"
        )
    except:
        await message.answer("❌ Enter a valid number like <code>30</code>", parse_mode="HTML")


@dp.message(PromoFSM.waiting_min_dep)
async def msg_promo_min(message: Message, state: FSMContext):
    try:
        val = float(message.text.strip())
        await state.update_data(promo_min=val)
        await state.set_state(PromoFSM.waiting_max_dep)
        await message.answer(
            f"✅ Min deposit: <b>{val}</b>\n\nStep 3/5: Enter <b>maximum deposit</b> amount\nExample: <code>10000</code>",
            parse_mode="HTML"
        )
    except:
...
@dp.message(PromoFSM.waiting_max_dep)
async def msg_promo_max(message: Message, state: FSMContext):
    try:
        val = float(message.text.strip())
        await state.update_data(promo_max=val)
        await state.set_state(PromoFSM.waiting_hours)
        await message.answer(
            f"✅ Max deposit: <b>{val}</b>\n\nStep 4/5: Enter <b>expiry hours</b>\nExample: <code>24</code> for 24 hours",
            parse_mode="HTML"
        )
    except:
        await message.answer("❌ Enter a valid number", parse_mode="HTML")
...
@dp.message(PromoFSM.waiting_hours)
async def msg_promo_hours(message: Message, state: FSMContext):
    try:
        val = int(message.text.strip())
        await state.update_data(promo_hours=val)
        await state.set_state(PromoFSM.waiting_max_uses)
        await message.answer(
            f"✅ Expires in: <b>{val}h</b>\n\nStep 5/5: Enter <b>max activations</b>\nExample: <code>50</code>",
            parse_mode="HTML"
        )
    except:
        await message.answer("❌ Enter a valid number", parse_mode="HTML")
...
@dp.message(PromoFSM.waiting_max_uses)
async def msg_promo_uses(message: Message, state: FSMContext):
    try:
        val = int(message.text.strip())
        await state.update_data(promo_uses=val)
        await state.set_state(PromoFSM.waiting_min_withdraw)
        await message.answer(
            f"✅ Max uses: <b>{val}</b>\n\nStep 6/6: Enter <b>minimum withdrawal</b> for promo users\nExample: <code>500</code>\n<i>Users who used this promo must withdraw at least this amount</i>",
            parse_mode="HTML"
        )
    except:
        await message.answer("❌ Enter a valid number", parse_mode="HTML")
...
@dp.message(PromoFSM.waiting_min_withdraw)
async def msg_promo_min_withdraw(message: Message, state: FSMContext):
    try:
        val = float(message.text.strip())
        data = await state.get_data()
        await state.clear()
        ...
        promo = await create_promo(
            code=code,
            bonus_pct=data["promo_bonus"],
            min_deposit=data["promo_min"],
            max_deposit=data["promo_max"],
            expiry_hours=data["promo_hours"],
        ...
        await message.answer(
            ...
            f"Bonus: +{promo['bonus_pct']}%\n"
            f"Min deposit: {promo['min_deposit']} | Max: {promo['max_deposit']}\n"
            f"Min withdrawal: <b>{val}</b>\n"
            f"Expires: {data['promo_hours']}h | Max uses: {data['promo_uses']}\n\n"
            f"Use Broadcast button to send to all users!",
            parse_mode="HTML", reply_markup=back_kb("admin_promo_panel")
        )
    except:
...
@dp.callback_query(F.data.startswith("admin_promo_deact_"))
async def cb_admin_promo_deact(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    code = callback.data.replace("admin_promo_deact_", "")
    await deactivate_promo(code)
    await callback.answer(f"✅ {code} deactivated!", show_alert=True)
    await cb_admin_promo_panel(callback, None)


@dp.callback_query(F.data == "admin_promo_broadcast")
async def cb_admin_promo_broadcast(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    await callback.answer()
    all_promos = await get_all_promos()
    active = [p for p in all_promos.values() if p.get("active") and
              ...
              len(p["used_by"]) < p["max_uses"]]
    if not active:
        await callback.message.answer("No active promo to broadcast!")
        return
    promo = active[0]
    uses_left = promo["max_uses"] - len(promo["used_by"])
    msg = (
        f"🎁 <b>SPECIAL PROMO BONUS!</b>\n\n"
        f"Activate <code>{promo['code']}</code> and get <b>+{promo['bonus_pct']}%</b> on your deposit!\n\n"
        f"Min deposit: <b>{promo['min_deposit']:,.2f}</b> Tokens\n"
        f"Max deposit: <b>{promo['max_deposit']:,.2f}</b> Tokens\n\n"
        f"⏰ Limited time only!\n"
        f"🔢 Only <b>{uses_left}</b> activations left!\n\n"
        f"Go to Wallet → Deposit → Promo Code 👇"
    )
...
_loser_board_cache: dict = {"entries": None, "ts": 0}

@dp.callback_query(F.data == "menu_loser_board")
async def cb_loser_board(callback: CallbackQuery):
    await callback.answer()
    enabled = await db.get_setting("loser_board_enabled")
    if enabled == "0":
        await callback.answer("Loser board is disabled.", show_alert=True); return

    import time
    now = time.time()
    cache_ttl = 3600  # 1 hour

    count = int(await db.get_setting("loser_board_count") or "4")
    max_amount = float(await db.get_setting("loser_board_max_amount") or "50000")

    # Use cache if fresh
    if (_loser_board_cache["entries"] is not None and
            now - _loser_board_cache["ts"] < cache_ttl):
        entries = _loser_board_cache["entries"]
    else:
        real_losers = await db.get_top_losers(count)
        entries = generate_loser_board(real_losers, count, max_amount)
        _loser_board_cache["entries"] = entries
        _loser_board_cache["ts"] = now

    # Show next refresh time
    next_ts = _loser_board_cache["ts"] + cache_ttl
    from datetime import datetime
    next_str = datetime.fromtimestamp(next_ts).strftime("%H:%M")
    text = loser_board_text(entries, next_str)

    from aiogram.utils.keyboard import InlineKeyboardBuilder as IKB
    from aiogram.types import InlineKeyboardButton as IKBtn
    builder = IKB()
    builder.row(IKBtn(text="🔙 Back", callback_data="menu_leaderboard"))
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=builder.as_markup())
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup())
...
@dp.callback_query(F.data == "admin_missions_panel")
async def cb_admin_missions_panel(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS and not await db.is_sub_admin(callback.from_user.id):
        await callback.answer("No permission.", show_alert=True); return
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    builder = InlineKeyboardBuilder()
    text_lines = [f"📋 <b>MISSION CONTROL</b>\n{SEP}"]
    for m in MISSIONS:
        val = await db.get_setting(m["setting_key"])
        status = "🟢" if val != "0" else "🔴"
        text_lines.append(f"{status} {m['icon']} {m['name']} ({m['type']}) — {m['reward']}T")
        builder.button(text=f"{status} {m['name']}", callback_data=f"mission_toggle_{m['id']}")
    builder.adjust(2)
...
@dp.callback_query(F.data.startswith("mission_toggle_"))
async def cb_mission_toggle(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS and not await db.is_sub_admin(callback.from_user.id):
        await callback.answer("No permission.", show_alert=True); return
    mission_id = callback.data.replace("mission_toggle_", "")
    mission = MISSION_BY_ID.get(mission_id)
    if not mission:
        await callback.answer("Mission not found.", show_alert=True); return
    current = await db.get_setting(mission["setting_key"])
    new_val = "0" if current != "0" else "1"
    await db.set_setting(mission["setting_key"], new_val)
    status = "ON 🟢" if new_val == "1" else "OFF 🔴"
    await callback.answer(f"{mission['name']}: {status}", show_alert=True)
    await cb_admin_missions_panel(callback)
...
async def cb_live_feed(callback: CallbackQuery):
    ...
    mix_fake = (await db.get_setting("live_feed_mix_fake")) != "0"
...
async def cb_pf_change_seed(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "🎯 <b>Change Client Seed</b>\nSend your custom client seed (any text):\nOr send <code>auto</code> to generate one.",
...
@dp.callback_query(F.data == "pf_verify")
async def cb_pf_verify(callback: CallbackQuery):
    ss, cs, nonce = await get_or_create_seed_pair(callback.from_user.id, db.db_path)
    text = provably_fair_info_text(hash_server_seed(ss), cs, nonce)
    await callback.message.answer(text, parse_mode="HTML", reply_markup=provably_fair_kb())
    await callback.answer()
...
async def cb_pvp_list(callback: CallbackQuery):
    ...
    for d in duels[:8]:
        game_emoji = GAME_EMOJIS.get(d["game_type"], "⚔️")
        text += f"{game_emoji} <b>{d['creator_name']}</b> — {d['bet']:,.0f} T\n"
        builder.button(
            text=f"⚔️ Join {d['creator_name']} ({d['bet']:,.0f}T)",
            callback_data=f"pvp_join_{d['id']}"
        )
    builder.adjust(1)
    builder.button(text="🔙 Back", callback_data="menu_pvp")
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=builder.as_markup())
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup())
