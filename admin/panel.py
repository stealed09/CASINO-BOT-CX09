...
async def cmd_tip(message: Message, bot: Bot):
    """Admin tip tokens to user: /tip @username amount  OR  /tip user_id amount"""
    parts = message.text.split()
    if len(parts) < 3:
        await message.answer("Usage: <code>/tip @username 500</code>", parse_mode="HTML"); return
    try:
        ...
        if not user:
            try:
                ...
                user = None
        if not user:
            await message.answer(error_text(f"User not found: {target_str}"), parse_mode="HTML"); return
        target = user["user_id"]
        await db.update_token_balance(target, amount)
        ...
        await message.answer(error_text(f"Error: {e}"), parse_mode="HTML")
