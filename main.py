...
async def cmd_tip_cmd(message: Message):
    """
    Group:   /tip 500  (tag the user in the same message)
    Private: /tip 500  (reply to user's message) or /tip user_id 500
    """
    if message.from_user.id not in ADMIN_IDS and not await db.is_sub_admin(message.from_user.id):
        return
    ...
    # Amount is always parts[1]
    try:
        amount = float(parts[1])
        if amount <= 0: raise ValueError
    except:
        await message.reply("Usage: <code>/tip 500</code> (tag the user while sending)", parse_mode="HTML")
        return

    target_id = None
    target_name = None

    # 1. Check for tagged user in entities (group tag)
    if message.entities:
    ...
    # 2. Fallback — private: /tip user_id 500
    if not target_id:
    ...
    if not user:
        await message.reply(error_text("Tagged user is not registered in bot."), parse_mode="HTML")
        return
    ...
    await message.reply(
        success_text(f"🎁 Tipped <b>{amount:,.0f}</b> Tokens to <b>{target_name}</b>!"),
        parse_mode="HTML"
    )
    try:
        await bot.send_message(target_id, success_text(f"🎁 Admin tipped you <b>{amount:,.0f}</b> Tokens!"), parse_mode="HTML")
    except:
        pass
