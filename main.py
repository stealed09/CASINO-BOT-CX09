...
async def msg_upi_promo(message: Message, state: FSMContext):
    ...
    # Save promo and clear FSM state before generating QR
    if code:
        await db.set_setting(f"pending_promo_{message.from_user.id}", code)
    await state.clear()
    await start_upi_deposit(message.from_user.id, bot, amount, message.from_user.id)
...
async def cb_upi_promo_skip(callback: CallbackQuery, state: FSMContext):
    ...
    await start_upi_deposit(callback.from_user.id, bot, amount, callback.from_user.id)
