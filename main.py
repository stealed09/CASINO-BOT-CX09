...
from games.pvp import (
    ...
    set_dice_roll, get_pending_duel_for_user
)
...
async def cb_pvp_join(callback: CallbackQuery):
    ...
    if duel["game_type"] == "dice":
        # Ask both players to roll dice
        creator_id = duel["creator_id"]
        opp_name = callback.from_user.first_name or str(user_id)
        roll_msg = (
            f"⚔️ <b>DUEL STARTED!</b>\n{SEP}\n"
            f"🎲 Both players: send a 🎲 dice emoji NOW!\n"
            f"👇 Tap to copy:\n<code>🎲</code>"
        )
        await callback.message.answer(roll_msg, parse_mode="HTML")
        try:
            await bot.send_message(creator_id, roll_msg, parse_mode="HTML")
        except:
            pass
    else:
        # Coinflip / highroll — resolve immediately
...
async def handle_dice_roll(message: Message):
    """Handle real Telegram dice rolls for PVP duels."""
    user_id = message.from_user.id
    roll_value = message.dice.value
    duel = get_pending_duel_for_user(user_id)
    if not duel:
        return  # Not in a dice duel, ignore
    duel_id = duel["id"]
    result = set_dice_roll(duel_id, user_id, roll_value)
    if result is None:
        await message.reply(f"🎲 Roll recorded: <b>{roll_value}</b>\nWaiting for opponent...", parse_mode="HTML")
        return
    # Both rolled — resolve
    resolved = resolve_duel(
        duel_id,
        creator_roll=result.get("pending_creator_roll"),
        opponent_roll=result.get("pending_opponent_roll")
    )
    if not resolved:
        return
    winner_id = resolved["winner_id"]
