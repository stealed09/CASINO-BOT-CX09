...
from ui.keyboards import (
    ...
    admin_panel_kb, admin_settings_kb, admin_redeems_kb, admin_balance_kb,
    approve_reject_deposit_kb, approve_reject_withdraw_kb,
    bonus_claim_kb, redeem_menu_kb
)
...
async def cmd_cf(message: Message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Usage: `/cf <amount>`\nExample: `/cf 100`", parse_mode="Markdown"); return
    amount, err = validate_amount(parts[1])
    if err:
...
async def cmd_tip(message: Message):
    ...
    if len(parts) < 3:
        await message.answer(
            f"💸 *TIP*\n{SEP}\n"
            f"Usage: `/tip @username amount` or `/tip user_id amount`\n"
            f"Example: `/tip @john 50` or `/tip 123456789 50`",
            parse_mode="Markdown"
        )
        return
    ...
    if amount <= 0:
        await message.answer(error_text("Amount must be positive."), parse_mode="Markdown"); return
    ...
    if sender["balance"] < amount:
        await message.answer(
            error_text(f"Insufficient balance!\nYour balance: {format_balance(sender['balance'])}"),
            parse_mode="Markdown"
        ); return
    ...
    if target_raw.startswith("@"):
        ...
        try:
            target = await db.get_user(int(target_raw))
        except ValueError:
            target = await db.get_user_by_username(target_raw)

    if not target:
        await message.answer(error_text("User not found. They must have used the bot before."), parse_mode="Markdown")
        return

    if target["user_id"] == user_id:
        await message.answer(error_text("You can't tip yourself!"), parse_mode="Markdown")
        return

    await db.update_balance(user_id, -amount)
    await db.update_balance(target["user_id"], amount)
    await db.add_transaction(user_id, "tip_sent", amount)
    await db.add_transaction(target["user_id"], "tip_received", amount)
    ...
    await message.answer(
        success_text(
            ...
            f"👤 To: {target_name}\n"
            f"💰 Amount: ₹{amount:,.2f}\n"
            f"💳 Your new balance: ₹{sender['balance'] - amount:,.2f}"
        ),
        parse_mode="Markdown",
    ...
    try:
        await bot.send_message(
            ...
            f"👤 From: {sender_name}\n"
            f"💰 Amount: *+₹{amount:,.2f}*\n"
            f"💳 Your balance: ₹{target['balance'] + amount:,.2f}",
            parse_mode="Markdown",
            reply_markup=back_kb()
        )
    except:
    ...
    logger.info(f"Tip: {user_id} -> {target['user_id']} ₹{amount}")
...
async def cmd_gencode(message: Message):
    ...
    if len(parts) < 2:
        await message.answer(
            f"Usage: `/gencode amount` or `/gencode amount MYCODE`\n"
            f"Example: `/gencode 100` or `/gencode 500 PROMO500`",
            parse_mode="Markdown"
        )
        return
    ...
    await db.create_redeem_code(code, amount, message.from_user.id)
    await message.answer(
        success_text(
            f"🎟️ Redeem Code Created!\n"
            f"📌 Code: `{code}`\n"
            f"💰 Amount: ₹{amount:,.2f}\n"
            f"🔁 Single use only"
        ),
...
async def cmd_redeem(message: Message):
    parts = message.text.split()
    if len(parts) >= 2:
        await _process_redeem(message, parts[1])
        return
    await message.answer(
        f"🎟️ *REDEEM CODE*\n{SEP}\n"
...
async def _process_redeem(message: Message, code: str):
    ...
    if not record:
        await message.answer(error_text("Invalid redeem code!"), parse_mode="Markdown", reply_markup=back_kb())
        return

    if record["used_by"] is not None:
        await message.answer(error_text("This code has already been used!"), parse_mode="Markdown", reply_markup=back_kb())
        return

    success = await db.use_redeem_code(code, user_id)
    if not success:
        await message.answer(error_text("Code already redeemed or invalid!"), parse_mode="Markdown", reply_markup=back_kb())
        return

    amount = record["amount"]
    await db.update_balance(user_id, amount)
    await db.add_transaction(user_id, "redeem", amount)

    await message.answer(
        success_text(
            f"🎟️ Code Redeemed Successfully!\n"
            f"📌 Code: `{code}`\n"
    ...
    logger.info(f"Redeem: user={user_id} code={code} amount={amount}")
        ...
        try:
            if screenshot_id:
                await bot.send_photo(
                    admin_id, photo=screenshot_id, caption=caption,
...
async def redeem_code_input(message: Message, state: FSMContext):
    await state.clear()
    await _process_redeem(message, message.text.strip())
...
@dp.message(AdminFSM.waiting_gencode)
async def admin_gencode_input(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        await state.clear(); return
    await state.clear()
    parts = (message.text or "").strip().split()
    if not parts:
        await message.answer(error_text("Invalid input."), parse_mode="Markdown"); return
    try:
        amount = float(parts[0])
    except:
        await message.answer(error_text("First value must be a number (amount)."), parse_mode="Markdown"); return
    if amount <= 0:
        await message.answer(error_text("Amount must be positive."), parse_mode="Markdown"); return
    if len(parts) >= 2:
        code = parts[1].upper()
    ...
    await message.answer(
        ...
        parse_mode="Markdown",
        reply_markup=back_kb("admin_redeems")
    )


@dp.message(AdminFSM.waiting_tip)
async def admin_tip_input(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        await state.clear(); return
    await state.clear()
    parts = (message.text or "").strip().split()
    if len(parts) < 2:
        await message.answer(error_text("Send: `user_id amount`"), parse_mode="Markdown"); return
    try:
        target_id, amount = int(parts[0]), float(parts[1])
    except:
        await message.answer(error_text("Invalid format. Use: `user_id amount`"), parse_mode="Markdown"); return
    if amount <= 0:
        await message.answer(error_text("Amount must be positive."), parse_mode="Markdown"); return
    ...
    await db.add_transaction(target_id, "tip_received", amount, "admin_tip")
    await message.answer(
        success_text(f"💸 Tipped ₹{amount:,.2f} to `{target_id}`"),
        parse_mode="Markdown", reply_markup=back_kb("admin_panel")
    )
    try:
        await bot.send_message(
            target_id,
            f"🎁 *ADMIN TIP!*\n{SEP}\n💰 You received *₹{amount:,.2f}* from admin!",
            parse_mode="Markdown"
        )
    except:
        pass
...
async def cb_redeem_menu(callback: CallbackQuery):
    try:
        await callback.message.edit_text(
            f"🎟️ *REDEEM CODE*\n{SEP}\n"
            f"Enter your redeem code to get free balance!\n\n"
            f"Use: `/redeem CODE` or tap below:",
            parse_mode="Markdown",
            reply_markup=redeem_menu_kb()
        )
    except:
        await callback.message.answer(
            f"🎟️ Enter your code using `/redeem CODE`",
...
async def cb_redeem_enter(callback: CallbackQuery, state: FSMContext):
    try:
        await callback.message.edit_text(
            f"🎟️ Send your redeem code now:",
            parse_mode="Markdown", reply_markup=back_kb("menu_redeem")
        )
    except:
        await callback.message.answer("Send your redeem code:", reply_markup=back_kb("menu_redeem"))
...
async def cb_game(callback: CallbackQuery):
    cmd = callback.data[5:]
    cmd_map = {"coinflip": "cf"}
    display_cmd = cmd_map.get(cmd, cmd)
    try:
        await callback.message.edit_text(
            f"Use: `/{display_cmd} <amount>`\nExample: `/{display_cmd} 100`",
...
async def cb_adm_redeems(callback: CallbackQuery):
    ...
    if not codes:
        try:
            await callback.message.edit_text(
                f"🎟️ *REDEEM CODES*\n{SEP}\nNo codes yet.",
                parse_mode="Markdown", reply_markup=admin_redeems_kb()
            )
        except:
            await callback.message.answer(
                f"🎟️ *REDEEM CODES*\n{SEP}\nNo codes yet.",
    ...
    for c in codes:
        ...
        lines.append(f"`{c['code']}` — ₹{c['amount']:,.2f} — {status}")
    ...
    try:
        await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=admin_redeems_kb())
    except:
        await callback.message.answer(text, parse_mode="Markdown", reply_markup=admin_redeems_kb())
    await callback.answer()

@dp.callback_query(F.data == "admin_gencode")
async def cb_adm_gencode(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Admin only!", show_alert=True); return
    await state.set_state(AdminFSM.waiting_gencode)
    try:
        await callback.message.edit_text(
            f"🎟️ *GENERATE REDEEM CODE*\n{SEP}\n"
            f"Send the amount:\nExample: `500`\n\n"
            f"Or send `amount CODE` for custom code:\nExample: `500 PROMO500`",
            parse_mode="Markdown", reply_markup=back_kb("admin_redeems")
        )
    except:
        await callback.message.answer(
            "Send amount (e.g. `500`) or `amount MYCODE`:",
            parse_mode="Markdown", reply_markup=back_kb("admin_redeems")
        )
    await callback.answer()

@dp.callback_query(F.data == "admin_tip")
async def cb_adm_tip(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Admin only!", show_alert=True); return
    await state.set_state(AdminFSM.waiting_tip)
    try:
        await callback.message.edit_text(
            f"💸 *ADMIN TIP*\n{SEP}\n"
        ...
        await callback.message.answer(
            "Send `user_id amount`:", parse_mode="Markdown", reply_markup=back_kb("admin_panel")
        )
    await callback.answer()
...
async def cb_adm_balance(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"💰 *MANAGE BALANCE*\n{SEP}\n"
            f"➕ Add: `/addbalance user_id amount`\n"
            f"➖ Remove: `/removebalance user_id amount`",
            parse_mode="Markdown", reply_markup=admin_balance_kb()
        )
    except:
        await callback.message.answer(
            "Use `/addbalance` or `/removebalance`",
            parse_mode="Markdown", reply_markup=admin_balance_kb()
        )
    await callback.answer()
