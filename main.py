...
async def cb_withdraw_crypto_select(callback: CallbackQuery, state: FSMContext):
    """Show currency selection menu for crypto withdrawal."""
    cryptos = await db.get_all_cryptos()
    if not cryptos:
        await callback.answer("No crypto withdrawal options available.", show_alert=True); return
    ...
    builder.row(InlineKeyboardButton(text="🔙 Back", callback_data="wallet_withdraw"))
    try:
        await callback.message.edit_text(
            f"₿ <b>CRYPTO WITHDRAWAL</b>\n{SEP}\n"
            f"Select your withdrawal currency:",
            parse_mode="HTML", reply_markup=builder.as_markup()
        )
    except:
        await callback.message.answer("Select currency:", reply_markup=builder.as_markup())
    await callback.answer()


@dp.callback_query(F.data.startswith("withdraw_crypto_"))
async def cb_withdraw_crypto(callback: CallbackQuery, state: FSMContext):
    if callback.data == "withdraw_crypto_select":
...
async def cb_admin_regen_code(callback: CallbackQuery, state: FSMContext):
    ...
    if not active:
        await callback.answer("No active codes to regenerate!", show_alert=True); return
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    builder = InlineKeyboardBuilder()
    for c in active[:10]:
        builder.row(InlineKeyboardButton(
            text=f"♻️ {c['code']} ({c['token_amount']:,.0f}T)",
            callback_data=f"regen_code_{c['code']}"
        ))
    builder.row(InlineKeyboardButton(text="🔙 Back", callback_data="admin_panel"))
    try:
        await callback.message.edit_text(
            f"♻️ <b>REGENERATE CODE</b>\n{SEP}\nSelect a code to regenerate:",
            parse_mode="HTML", reply_markup=builder.as_markup()
        )
    except:
...
async def cmd_tip_cmd(message: Message):
    ...
    # If only /tip and no args but there's a text_mention entity → extract user from entity
    if len(parts) < 3 and message.entities:
        for ent in message.entities:
            if ent.type == "text_mention" and ent.user:
                # /tip [mention] 500 → try to get amount from last part
                try:
                    amount = float(parts[-1]) if len(parts) >= 2 else None
                except:
                    amount = None
                if not amount:
                    await message.answer("Usage: <code>/tip @username 500</code>", parse_mode="HTML")
                    return
                target_user = ent.user
                user = await db.get_user(target_user.id)
                if not user:
                    await message.answer(error_text(f"User not registered in bot."), parse_mode="HTML")
                    return
                await db.update_token_balance(target_user.id, amount)
                await db.add_transaction(target_user.id, "tip_received", amount, "completed", currency="TOKEN")
                uname = target_user.username or target_user.first_name or str(target_user.id)
                await message.answer(success_text(f"🎁 Tipped <b>{amount:,.4f}</b> Tokens to @{uname}!"), parse_mode="HTML")
                try:
                    await bot.send_message(target_user.id, success_text(f"🎁 Admin tipped you <b>{amount:,.4f}</b> Tokens!"), parse_mode="HTML")
                except:
                    pass
                return
