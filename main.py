...
async def cb_admin_promo_panel(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("No access.", show_alert=True); return
    await callback.answer()
    all_promos = await get_all_promos()
    ...
    for code, p in list(all_promos.items())[-5:]:
        ...
        lines.append(f"{status} <code>{code}</code> — {p['bonus_pct']}% | {len(p['used_by'])}/{p['max_uses']} used")
    lines.append("\nCreate a new promo code:")
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="➕ Create Promo", callback_data="admin_promo_create"))
    for code, p in list(all_promos.items())[-3:]:
        if p.get("active"):
            builder.row(InlineKeyboardButton(text=f"❌ Deactivate {code}", callback_data=f"admin_promo_deact_{code}"))
    builder.row(InlineKeyboardButton(text="📢 Broadcast Active Promo", callback_data="admin_promo_broadcast"))
...
async def msg_promo_uses(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            f"Min: {promo['min_deposit']} | Max: {promo['max_deposit']}\n"
            f"Expires: {data['promo_hours']}h | Max uses: {val}\n\n"
