...
async def slots_bet_input(message: Message, state: FSMContext):
    ...
    if net > 0:
        ...
        await db.record_game_result(user_id, True, net)
        await db.update_mission_progress(user_id, wins=1)
        ...
        await db.record_game_result(user_id, False, 0)
    await state.clear()
...
async def cb_pvp_create(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"{names.get(game_type, '⚔️')} — Send your bet amount:",
...
async def pvp_bet_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply(error_text("❌ Invalid amount."), parse_mode="HTML"); return
    user = await db.get_user(user_id)
    if not user or user["token_balance"] < bet:
        await message.reply(error_text(f"❌ Insufficient balance! You have {user['token_balance']:,.4f} Tokens."), parse_mode="HTML")
        return
    ...
    sent = await message.reply(text, parse_mode="HTML", reply_markup=pvp_join_kb(duel_id))
    await state.clear()
...
async def cb_pvp_join(callback: CallbackQuery):
    ...
    await db.record_game_result(winner_id, True, net_prize)
    await db.record_game_result(loser_id, False, 0)
...
async def cb_admin_rain(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🌧️ Send total rain amount in Tokens:", reply_markup=back_kb("admin_panel"))
    await callback.answer()
...
async def rain_amount_input(message: Message, state: FSMContext):
    ...
    await message.reply("How many max winners? (e.g. 20):")
...
async def rain_winners_input(message: Message, state: FSMContext):
    try:
        ...
        await message.reply(error_text("❌ Invalid number."), parse_mode="HTML"); return
    data = await state.get_data()
    amount = data["rain_amount"]
    rain = create_rain(amount, winners, duration_seconds=60, triggered_by="Admin")
    await state.clear()
    # Get group chat IDs from settings
    ...
    if group_id:
        try:
            ...
            pass
    await message.reply(f"🌧️ Rain started! Amount: {amount:,.0f} | Winners: {winners}", parse_mode="HTML")
...
NEW_FEATURE_TOGGLES = [
    "aset_vip_enabled", "aset_rakeback_enabled", "aset_missions_enabled",
    "aset_lootbox_enabled", "aset_crash_enabled", "aset_slots_enabled",
    "aset_live_feed_enabled", "aset_pvp_house_fee", "aset_rakeback_weekly_pct",
]


@dp.callback_query(F.data.in_(set(NEW_FEATURE_TOGGLES)))
async def cb_new_feature_toggle(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS and not await db.is_sub_admin(callback.from_user.id):
        await callback.answer("No permission.", show_alert=True); return
    key_map = {
        "aset_vip_enabled": ("vip_enabled", "Enter 1 to enable or 0 to disable VIP system:"),
        "aset_rakeback_enabled": ("rakeback_enabled", "Enter 1 to enable or 0 to disable Rakeback:"),
        "aset_missions_enabled": ("missions_enabled", "Enter 1 to enable or 0 to disable Missions:"),
        "aset_lootbox_enabled": ("lootbox_enabled", "Enter 1 to enable or 0 to disable Lootboxes:"),
        "aset_crash_enabled": ("crash_enabled", "Enter 1 to enable or 0 to disable Crash game:"),
        "aset_slots_enabled": ("slots_enabled", "Enter 1 to enable or 0 to disable Slots:"),
        "aset_live_feed_enabled": ("live_feed_enabled", "Enter 1 to enable or 0 to disable Live Feed:"),
    ...
    db_key, prompt = key_map[callback.data]
    await state.update_data(setting_key=db_key)
    await state.set_state(AdminFSM.waiting_value)
    await callback.message.answer(prompt, reply_markup=back_kb("admin_new_features"))
    await callback.answer()
