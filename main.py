...
async def cb_menu_promo(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    # Check if any active promo exists
    ...
    if active:
        promo = active[0]
        text = promo_info_text(promo) + "\n\nSend the promo code to activate:"
    else:
        text = "🎁 <b>PROMO CODE</b>\n\nNo active promo codes right now.\nCheck back later!"
    await state.set_state(PromoFSM.waiting_code)
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=back_kb("wallet_deposit"))
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=back_kb("wallet_deposit"))
