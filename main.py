...
from ui.keyboards import (
    ...
    crypto_manual_select_kb, withdraw_method_kb, nowpayments_currency_kb,
    support_reply_kb
)
...
SETTING_PROMPTS = {
    "aset_minwd":               ("min_withdrawal_tokens", "Enter minimum withdrawal in *Tokens* (e.g. 100):"),
    "aset_wdtoggle":            ("withdraw_enabled", "Send *1* to enable or *0* to disable withdrawals:"),
    "aset_weekly":              ("weekly_bonus_tokens", "Enter weekly bonus in *Tokens*:"),
    "aset_monthly":             ("monthly_bonus_tokens", "Enter monthly bonus in *Tokens*:"),
    "aset_bonusmode":           ("bonus_mode", "Send *fixed* or *wagered*:"),
...
async def _send_main_menu(message: Message, user_id: int):
    ...
    try:
        await message.edit_text(text, parse_mode="Markdown", reply_markup=main_menu_kb(is_admin))
    except:
        await message.answer(text, parse_mode="Markdown", reply_markup=main_menu_kb(is_admin))
            ...
            if not user.get("bonus_warned"):
                ...
                try:
                    await bot.send_message(
                        ...
                        f"⏰ You have *1 hour* or bonus progress resets!",
                        parse_mode="Markdown"
                    )
                except:
                    pass
...
async def cb_main_menu(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=main_menu_kb(is_admin))
    except:
        await callback.message.answer(text, parse_mode="Markdown", reply_markup=main_menu_kb(is_admin))
...
async def cb_wallet(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            wallet_text(user), parse_mode="Markdown", reply_markup=wallet_kb()
        )
    except:
        await callback.message.answer(wallet_text(user), parse_mode="Markdown", reply_markup=wallet_kb())
    await callback.answer()
...
async def cb_referral(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer(referral_text(user, ref_count, me.username), parse_mode="Markdown")
    await callback.answer()
...
async def cb_bonus(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            bonus_text(user, weekly, monthly, mode),
            parse_mode="Markdown", reply_markup=bonus_claim_kb(can_w, can_m)
        )
    except:
        await callback.message.answer(bonus_text(user, weekly, monthly, mode), parse_mode="Markdown", reply_markup=bonus_claim_kb(can_w, can_m))
    await callback.answer()
...
async def cb_bonus_claim(callback: CallbackQuery):
    ...
    can_m = await can_claim_bonus(user, "monthly")
    try:
        await callback.message.edit_text(bonus_text(user, weekly, monthly, mode), parse_mode="Markdown", reply_markup=bonus_claim_kb(can_w, can_m))
    except:
        pass
...
async def cb_history(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            history_text(txns), parse_mode="Markdown", reply_markup=back_kb()
        )
    except:
...
async def msg_admin_support_reply(message: Message, state: FSMContext):
    ...
    if not ticket:
        await message.answer(error_text("Ticket not found."), parse_mode="Markdown"); return
    try:
        await bot.send_message(
            ticket["user_id"],
        ...
        await message.answer(success_text(f"Reply sent for ticket #{tid}"), parse_mode="Markdown")
...
async def msg_redeem_code(message: Message, state: FSMContext):
    ...
    if result is None:
        await message.answer(error_text("Invalid code."), parse_mode="Markdown", reply_markup=back_kb("menu_redeem"))
    elif result == -1.0:
        await message.answer(error_text("Code already used."), parse_mode="Markdown", reply_markup=back_kb("menu_redeem"))
    else:
...
async def cb_leaderboard(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            ...
            parse_mode="Markdown", reply_markup=leaderboard_kb()
        )
    except:
        await callback.message.answer(leaderboard_text(entries, "lifetime"), parse_mode="Markdown", reply_markup=leaderboard_kb())
    await callback.answer()
...
async def cb_lb_filter(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer(leaderboard_text(entries, period), parse_mode="Markdown")
    await callback.answer()
...
async def cb_deposit_menu(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"💳 *DEPOSIT*\n{SEP}\n"
            f"🪙 Token Rates:\n"
            f"  🏦 INR: ₹1 = *{inr_rate} Token(s)*\n"
            f"  ⭐ Stars: 1★ = *{stars_rate} Token(s)*\n"
            f"  ₿ Crypto: varies by coin\n"
            f"🧾 Your Deposit Tax: *{dep_tax}%*\n"
            f"{SEP}\nChoose deposit method:",
            parse_mode="Markdown", reply_markup=deposit_method_kb()
        )
    except:
...
async def msg_stars_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(error_text("Enter a valid number of Stars (e.g. 100)"), parse_mode="Markdown"); return
    await state.clear()
...
async def msg_upi_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(error_text("Enter valid amount (e.g. 500)"), parse_mode="Markdown"); return
...
async def msg_upi_screenshot(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Screenshot received!\nNow send your *Transaction ID / UTR number*:",
        parse_mode="Markdown", reply_markup=back_kb("wallet_deposit")
    )
...
async def msg_upi_txn_id(message: Message, state: FSMContext):
    ...
    if not deposit:
        await message.answer(error_text("Deposit not found."), parse_mode="Markdown"); return
...
# ─── CRYPTO DEPOSIT (MANUAL) ──────────────────────────────────────────────────

@dp.callback_query(F.data == "deposit_crypto_manual")
async def cb_crypto_manual(callback: CallbackQuery):
    cryptos = await db.get_all_cryptos()
    if not cryptos:
        await callback.answer("No crypto addresses configured.", show_alert=True); return
    try:
        await callback.message.edit_text(
            f"₿ *MANUAL CRYPTO DEPOSIT*\n{SEP}\nChoose currency:",
            parse_mode="Markdown", reply_markup=crypto_manual_select_kb(cryptos)
        )
    except:
        await callback.message.answer("Choose crypto:", reply_markup=crypto_manual_select_kb(cryptos))
    await callback.answer()


@dp.callback_query(F.data.startswith("crypto_deposit_manual_"))
async def cb_crypto_manual_select(callback: CallbackQuery, state: FSMContext):
    symbol = callback.data.replace("crypto_deposit_manual_", "")
    ...
    rate = await db.get_setting(rate_key) or "?"
    ...
    try:
        await callback.message.edit_text(
            ...
            f"📊 Rate: 1 {symbol} = *{rate} Tokens*\n"
            f"🧾 Tax: *{dep_tax}%*\n\n"
            f"Enter amount of *{symbol}* you're sending:",
            parse_mode="Markdown", reply_markup=back_kb("wallet_deposit")
        )
    except:
        await callback.message.answer(f"Enter {symbol} amount:")
    await callback.answer()


@dp.message(DepositFSM.crypto_amount)
async def msg_crypto_amount(message: Message, state: FSMContext):
    try:
        amount = float(message.text.strip())
        if amount <= 0: raise ValueError
    except:
        await message.answer(error_text("Enter valid amount"), parse_mode="Markdown"); return
    data = await state.get_data()
    symbol = data.get("crypto_symbol")
    await state.update_data(crypto_dep_amount=amount)
    await state.set_state(DepositFSM.crypto_screenshot)

    crypto = await db.get_crypto(symbol)
    did = await db.create_deposit(
        message.from_user.id, "crypto",
        crypto_currency=symbol, crypto_amount=amount
    )
    await state.update_data(crypto_did=did)
...
@dp.message(DepositFSM.crypto_screenshot, F.photo)
async def msg_crypto_screenshot(message: Message, state: FSMContext):
    data = await state.get_data()
    did = data.get("crypto_did")
    screenshot_id = message.photo[-1].file_id
    await state.update_data(crypto_screenshot_id=screenshot_id)
    await state.set_state(DepositFSM.crypto_txn_id)
    await message.answer("Send your *Transaction Hash / TXID*:", parse_mode="Markdown")


@dp.message(DepositFSM.crypto_txn_id)
async def msg_crypto_txn_id(message: Message, state: FSMContext):
    data = await state.get_data()
    did = data.get("crypto_did")
    screenshot_id = data.get("crypto_screenshot_id", "")
    symbol = data.get("crypto_symbol")
    txn_id = message.text.strip()
    await state.clear()
    ...
    await message.answer(
        success_text(f"Crypto deposit submitted!\n🆔 #{did} | Txn: {txn_id}\n⏳ Admin will verify soon."),
        parse_mode="Markdown", reply_markup=back_kb()
    )
            ...
            if screenshot_id:
                await bot.send_photo(
                    ...
                    caption=(
                        f"₿ *CRYPTO DEPOSIT*\n{SEP}\n"
                        f"👤 @{uname} (`{message.from_user.id}`)\n"
                        f"🪙 {deposit.get('crypto_amount', 0):.6f} {symbol}\n"
                        f"🔖 Txn: {txn_id}\n🆔 #{did}"
                    ),
                    parse_mode="Markdown",
                    reply_markup=approve_reject_deposit_kb(did)
                ...
                await bot.send_message(
                    ...
                    f"₿ *CRYPTO DEPOSIT*\n{SEP}\n"
                    f"👤 @{uname} (`{message.from_user.id}`)\n"
                    f"🪙 {deposit.get('crypto_amount', 0):.6f} {symbol} | Txn: {txn_id}\n🆔 #{did}",
                    parse_mode="Markdown",
                    reply_markup=approve_reject_deposit_kb(did)
                )
        except Exception as e:
...
async def msg_nowpay_token_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(error_text("Enter valid token amount"), parse_mode="Markdown"); return
    await state.clear()
    try:
...
async def cb_nowpay_currency(callback: CallbackQuery):
    ...
    await callback.answer()
    try:
        await callback.message.edit_text(f"⏳ Creating payment...", parse_mode="Markdown")
    except:
        pass
    asyncio.create_task(
...
async def cb_withdraw_menu(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"💸 *WITHDRAW*\n{SEP}\n"
            f"🪙 Balance: *{user['token_balance']:,.4f} Tokens*\n"
            f"📉 Minimum: *{min_wd} Tokens*\n"
            f"🧾 Your Tax: *{wd_tax}%*\n"
            f"{SEP}\nChoose withdrawal method:",
            parse_mode="Markdown", reply_markup=withdraw_method_kb(cryptos)
        )
    except:
...
async def cb_withdraw_upi(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            f"🏦 *UPI WITHDRAWAL*\n{SEP}\n"
            f"🪙 Your Balance: *{user['token_balance']:,.4f} Tokens*\n"
            f"💵 ≈ ₹{user['token_balance'] / inr_rate:,.2f} (at ₹{inr_rate}/token)\n"
            f"📉 Minimum: *{min_wd} Tokens*\n"
            f"🧾 Tax: *{wd_tax}%*\n\n"
            f"Send: `token_amount UPI_ID`\nExample: `100 yourname@upi`",
            parse_mode="Markdown", reply_markup=back_kb("wallet_withdraw")
        )
    except:
...
async def msg_withdraw_upi(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(error_text("Send: `amount upi_id` e.g. `500 name@upi`"), parse_mode="Markdown"); return
    await state.clear()
...
async def msg_withdraw_crypto_amount(message: Message, state: FSMContext):
    ...
    crypto = await db.get_crypto(symbol)
    await message.answer(
        f"📬 Enter your *{symbol}* wallet address ({crypto['network']}):",
        parse_mode="Markdown", reply_markup=back_kb("wallet_withdraw")
    )
...
async def _play_game(message: Message, amount: float, play_fn, game_name: str):
    ...
    user = await db.get_user(user_id)
    if not user:
        await message.answer(error_text("Use /start first."), parse_mode="Markdown"); return
    if user.get("is_banned"):
        await message.answer(error_text("Account banned."), parse_mode="Markdown"); return
    ...
    if await db.is_balance_locked(user_id):
        await message.answer(error_text("Game in progress. Wait."), parse_mode="Markdown"); return
...
async def cmd_dice(message: Message):
    ...
    if len(parts) < 2:
        await message.answer("Usage: `/dice amount`\nExample: `/dice 100`", parse_mode="Markdown"); return
    try:
        amount = float(parts[1])
        if amount <= 0: raise ValueError
    except:
        await message.answer(error_text("Invalid amount"), parse_mode="Markdown"); return
...
async def cmd_basketball(message: Message):
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Usage: `/basketball amount`", parse_mode="Markdown"); return
    try:
        amount = float(parts[1])
    except:
        await message.answer(error_text("Invalid amount"), parse_mode="Markdown"); return
...
async def cmd_soccer(message: Message):
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Usage: `/soccer amount`", parse_mode="Markdown"); return
    try:
        amount = float(parts[1])
    except:
        await message.answer(error_text("Invalid amount"), parse_mode="Markdown"); return
...
async def cmd_bowling(message: Message):
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Usage: `/bowling amount`", parse_mode="Markdown"); return
    try:
        amount = float(parts[1])
    except:
        await message.answer(error_text("Invalid amount"), parse_mode="Markdown"); return
...
async def cmd_darts(message: Message):
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Usage: `/darts amount`", parse_mode="Markdown"); return
    try:
        amount = float(parts[1])
    except:
        await message.answer(error_text("Invalid amount"), parse_mode="Markdown"); return
...
async def cmd_limbo(message: Message):
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Usage: `/limbo amount`", parse_mode="Markdown"); return
    try:
        amount = float(parts[1])
    except:
        await message.answer(error_text("Invalid amount"), parse_mode="Markdown"); return
...
async def cmd_cf(message: Message, state: FSMContext):
    ...
    if not user or user["token_balance"] < amount:
        await message.answer(error_text("Insufficient Tokens."), parse_mode="Markdown"); return
...
async def cb_admin_panel(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"🔐 *ADMIN PANEL*\n{SEP}\n👥 Users: *{len(users)}* | 🪙 Tokens: *{total_tokens:,.4f}*",
...
async def cb_auser_action(callback: CallbackQuery, state: FSMContext):
    ...
    prompt_map = {
        "add": f"Enter tokens to ADD to user `{user_id}`:",
        "remove": f"Enter tokens to REMOVE from user `{user_id}`:",
        "set": f"Enter new token balance for user `{user_id}`:",
        "deptax": f"Enter custom deposit tax % for user `{user_id}` (or -1 to reset to global):",
        "wdtax": f"Enter custom withdraw tax % for user `{user_id}` (or -1 to reset to global):",
    }
    ...
    try:
        await callback.message.edit_text(
            prompt_map[action], parse_mode="Markdown",
...
async def msg_auser_action_value(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(error_text("Invalid number."), parse_mode="Markdown"); return
    ...
    if action == "add":
        ...
        await message.answer(success_text(f"Added {value:,.4f} Tokens to `{user_id}`"), parse_mode="Markdown")
        try:
            await bot.send_message(user_id, success_text(f"👑 Admin added *{value:,.4f} Tokens*!"), parse_mode="Markdown")
        except:
            pass
    elif action == "remove":
        ...
        await message.answer(success_text(f"Removed {value:,.4f} Tokens from `{user_id}`"), parse_mode="Markdown")
    elif action == "set":
        await db.set_token_balance(user_id, value)
        await message.answer(success_text(f"Set `{user_id}` balance to {value:,.4f} Tokens"), parse_mode="Markdown")
    elif action == "deptax":
        await db.set_user_custom_tax(user_id, deposit_tax=value)
        await message.answer(success_text(f"Deposit tax for `{user_id}` set to {value}%"), parse_mode="Markdown")
    elif action == "wdtax":
        await db.set_user_custom_tax(user_id, withdraw_tax=value)
        await message.answer(success_text(f"Withdraw tax for `{user_id}` set to {value}%"), parse_mode="Markdown")
...
async def cb_adm_broadcast(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            "📢 Use: `/broadcast your message`", parse_mode="Markdown",
...
async def cb_adm_redeems(callback: CallbackQuery):
    ...
    for c in codes:
        ...
        lines.append(f"`{c['code']}` — {c['token_amount']:,.4f} T — {status}")
    text = "\n".join(lines)
...
async def cb_crypto_detail(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            ...
            f"📬 Address: `{crypto['wallet_address']}`\n"
            f"🔘 Status: {status}",
            parse_mode="Markdown", reply_markup=admin_crypto_detail_kb(symbol, crypto["enabled"])
        )
    except:
...
async def msg_crypto_addr_update(message: Message, state: FSMContext):
    ...
    await message.answer(
        success_text(f"*{symbol}* address updated to:\n`{message.text.strip()}`"),
        parse_mode="Markdown", reply_markup=back_kb("admin_crypto")
    )
...
async def msg_crypto_add_address(message: Message, state: FSMContext):
    ...
    await message.answer(
        success_text(f"Added *{sym}* ({network})\nAddress: `{address}`"),
        parse_mode="Markdown", reply_markup=back_kb("admin_crypto")
    )
...
async def cmd_userinfo(message: Message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Usage: `/userinfo user_id_or_@username`", parse_mode="Markdown"); return
