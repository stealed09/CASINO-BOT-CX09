...
async def msg_upi_promo(message: Message, state: FSMContext):
    ...
    # Keep state as upi_screenshot so cb_upi_done works
    await state.set_state(DepositFSM.upi_screenshot)
    await start_upi_deposit(message.from_user.id, bot, amount, message.from_user.id, state)
...
async def cb_upi_promo_skip(callback: CallbackQuery, state: FSMContext):
    ...
    amount = data.get("upi_inr_amount", 0)
    await state.set_state(DepositFSM.upi_screenshot)
    await start_upi_deposit(callback.from_user.id, bot, amount, callback.from_user.id, state)
...
async def cb_crypto_promo_skip(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await start_oxapay_deposit(callback, token_amount)
