...
async def cb_admin_rain(callback: CallbackQuery, state: FSMContext):
    ...
    for amt in [500, 1000, 2000, 5000, 10000]:
        builder.button(text=f"{amt} T", callback_data=f"rain_amount_{amt}")
    builder.adjust(3)
    builder.row(__import__("aiogram").types.InlineKeyboardButton(text="🔙 Back", callback_data="admin_new_features"))
    await callback.message.answer(
        "🌧️ Select rain amount:",
        reply_markup=builder.as_markup()
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("rain_amount_"))
async def cb_rain_amount(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS and not await db.is_sub_admin(callback.from_user.id):
        await callback.answer("No permission.", show_alert=True); return
    amount = float(callback.data.replace("rain_amount_", ""))
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    builder = InlineKeyboardBuilder()
    for w in [10, 20, 30, 50]:
        builder.button(text=f"{w} winners", callback_data=f"rain_start_{amount}_{w}")
    builder.adjust(2)
    await callback.message.answer(
        "🌧️ Amount: " + f"{amount:,.0f}" + " Tokens\nSelect max winners:",
        reply_markup=builder.as_markup()
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("rain_start_"))
async def cb_rain_start(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS and not await db.is_sub_admin(callback.from_user.id):
        await callback.answer("No permission.", show_alert=True); return
    parts = callback.data.split("_")
    ...
    if group_id:
        try:
            ...
            pass
    await callback.message.answer(f"🌧️ Rain started! Amount: {amount:,.0f} | Max Winners: {winners}", parse_mode="HTML")
    await callback.answer()
