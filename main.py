...
from games.coinflip import prompt_coinflip, play_coinflip, coinflip_side_selected
...
async def cb_cf(callback: CallbackQuery):
    ...
    # Step 1: user chose heads or tails → cf_heads_AMOUNT / cf_tails_AMOUNT
    if parts[1] in ("heads", "tails"):
        choice = parts[1]
    ...
    # Step 2: user wants to reselect → cf_reselect_AMOUNT
    if parts[1] == "reselect":
        amount = float(parts[2])
        user = await db.get_user(callback.from_user.id)
        if not user or user["token_balance"] < amount:
            await callback.answer("❌ Insufficient Tokens!", show_alert=True); return
        from ui.keyboards import coinflip_choice_kb
        try:
            await callback.message.edit_text(
                f"🪙 <b>COIN FLIP</b>\n{SEP}\n"
                ...
                f"👑 = Heads   |   🦅 = Tails\n\nChoose your side:",
                parse_mode="HTML",
                reply_markup=coinflip_choice_kb(str(amount))
            )
        except: pass
        await callback.answer()
        return

    # Step 3: user pressed FLIP → cf_flip_AMOUNT_CHOICE
    if parts[1] == "flip":
        amount = float(parts[2])
        choice = parts[3]
        user = await db.get_user(callback.from_user.id)
