...
async def cb_pvp_join(callback: CallbackQuery):
    ...
    if game_type == "dice":
        # Ask both to roll — 30 sec timer
        roll_text = (
            f"⚔️ <b>DUEL STARTED!</b>\n{SEP}\n"
            f"vs <b>{opponent_name}</b> joined!\n\n"
        ...
        try:
            ...
            pass
        # Auto-expire rolls after 30 sec
        ...
        await _finish_pvp(resolved, callback.message, creator_id, user_id)
...
async def _finish_pvp(resolved: dict, message, creator_id: int, opponent_id: int):
    ...
    for uid in [creator_id, opponent_id]:
        try:
            ...
            pass
    # Notify group if duel was created there
    group_chat = resolved.get("chat_id") or active_duels.get(resolved.get("id", ""), {}).get("chat_id")
    if group_chat and group_chat != message.chat.id:
        try:
            await bot.send_message(group_chat, text, parse_mode="HTML")
        except:
            pass
