        ...
        if not user.get("bonus_eligible"):
            ...
            try:
                await bot.send_message(
                    user_id,
                    f"✅ <b>BONUS ELIGIBLE!</b>\n{SEP}\n"
                    f"We found <b>{tag_display}</b> in your profile! 🎉\n\n"
            ...
            await db.set_warn(user_id, 0, None)
            try:
                await bot.send_message(user_id, f"✅ <b>Tag Restored!</b> Keep <b>{tag_display}</b> in your profile!", parse_mode="HTML")
            except:
                pass
    else:
            ...
            if not user.get("bonus_warned"):
                ...
                try:
                    await bot.send_message(
                        ...
                        f"Add <b>{tag_display}</b> to your name.\n"
                        f"⏰ You have <b>1 hour</b> or bonus progress resets!",
                        parse_mode="HTML"
                    )
                except:
                    pass
                ...
                if warn_time_str and datetime.now() > datetime.fromisoformat(warn_time_str):
                    ...
                    try:
                        await bot.send_message(user_id, f"❌ <b>BONUS RESET</b>\n{SEP}\nAdd <b>{tag_display}</b> back to restart!", parse_mode="HTML")
                    except:
                        pass
...
async def cb_bonus(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer(bonus_text(user, weekly, monthly, mode), parse_mode="HTML", reply_markup=bonus_claim_kb(can_w, can_m))
    await callback.answer()
...
async def cb_bonus_claim(callback: CallbackQuery):
    ...
    can_m = await can_claim_bonus(user, "monthly")
    try:
        await callback.message.edit_text(bonus_text(user, weekly, monthly, mode), parse_mode="HTML", reply_markup=bonus_claim_kb(can_w, can_m))
    except:
        pass
...
async def cb_crypto_manual(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"₿ <b>MANUAL CRYPTO DEPOSIT</b>\n{SEP}\nChoose currency:",
            parse_mode="HTML", reply_markup=crypto_manual_select_kb(cryptos)
        )
    except:
...
async def cb_crypto_manual_select(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            f"₿ <b>{symbol} DEPOSIT</b>\n{SEP}\n"
            f"🌐 Network: <b>{crypto['network']}</b>\n"
            f"📬 Address: <code>{crypto['wallet_address']}</code>\n"
            f"📊 Rate: 1 {symbol} = <b>{rate} Tokens</b>\n"
            f"🧾 Tax: <b>{dep_tax}%</b>\n\n"
            f"Enter amount of <b>{symbol}</b> you're sending:",
            parse_mode="HTML", reply_markup=back_kb("wallet_deposit")
        )
    except:
...
async def msg_crypto_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"₿ <b>{symbol} DEPOSIT</b>\n{SEP}\n"
        f"💰 Amount: <b>{amount:.6f} {symbol}</b>\n"
        f"🌐 Network: <b>{crypto['network']}</b>\n"
        f"📬 Send to:\n<code>{crypto['wallet_address']}</code>\n\n"
        f"📸 After sending, upload <b>screenshot</b>:",
        parse_mode="HTML", reply_markup=back_kb("wallet_deposit")
    )
...
async def msg_crypto_screenshot(message: Message, state: FSMContext):
    ...
    await message.answer("Send your <b>Transaction Hash / TXID</b>:", parse_mode="HTML")
