...
async def cb_nowpay_currency(callback: CallbackQuery):
    # Format: np_pay_{currency}_{amount}  — split from right to get amount safely
    data = callback.data[len("np_pay_"):]  # strip prefix
    last_underscore = data.rfind("_")
    if last_underscore == -1:
        await callback.answer("Invalid data.", show_alert=True); return
    pay_currency = data[:last_underscore]
    try:
        token_amount = float(data[last_underscore + 1:])
    except:
