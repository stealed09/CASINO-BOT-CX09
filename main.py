...
async def cmd_tip_cmd(message: Message):
    ...
    if not target_id and message.reply_to_message and message.reply_to_message.from_user:
        ...
        if not rep.is_bot:
            target_id = rep.id
            target_name = rep.first_name or str(rep.id)
    ...
    if target_id == sender_id:
        await message.reply(error_text("❌ Aap khud ko tip nahi de sakte!"), parse_mode="HTML")
        return

    target_user = await db.get_user(target_id)
    if not target_user:
        await message.reply(error_text("❌ Yeh user bot mein registered nahi hai!"), parse_mode="HTML")
        return
    ...
    if is_admin:
        ...
        try:
            await bot.send_message(target_id, success_text(f"🎁 Admin ne aapko <b>{amount:,.4f}</b> Tokens tip kiye!"), parse_mode="HTML")
        except:
            pass
        return
    ...
    if tip_enabled != "1":
        await message.reply(error_text("❌ User tips abhi disabled hain."), parse_mode="HTML")
        return
    ...
    if amount < tip_min:
        await message.reply(error_text(f"❌ Minimum tip <b>{tip_min:,.0f}</b> Tokens hai."), parse_mode="HTML")
        return
    if tip_max > 0 and amount > tip_max:
        await message.reply(error_text(f"❌ Maximum tip <b>{tip_max:,.0f}</b> Tokens hai."), parse_mode="HTML")
        return

    sender = await db.get_user(sender_id)
    if not sender:
        await message.reply(error_text("❌ Pehle bot start karo!"), parse_mode="HTML")
        return
    if sender["token_balance"] < amount:
        await message.reply(
            error_text(f"❌ Balance kam hai! Aapke paas <b>{sender['token_balance']:,.4f}</b> Tokens hain."),
            parse_mode="HTML"
        )
        return
    ...
    await message.reply(
        ...
        f"🪙 Aapka Balance: <b>{new_balance:,.4f}</b>",
        parse_mode="HTML"
    )
    try:
        await bot.send_message(
            target_id,
            f"🎁 <b>AAPKO TIP MILA!</b>\n{SEP}\n"
