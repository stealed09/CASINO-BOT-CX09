...
from games.coinflip import prompt_coinflip, play_coinflip, COIN_EMOJI
...
async def cb_cf_pick(callback: CallbackQuery, state: FSMContext):
    """User picked Heads or Tails via inline button — now send them copyable coin emoji."""
    parts = callback.data.split("_")
    ...
    await state.update_data(cf_bet=amount, cf_choice=choice)
    await state.set_state(CoinflipFSM.waiting_emoji)
    await callback.message.edit_text(
        f"🪙 <b>COIN FLIP</b>\n{SEP}\n"
        f"💰 Bet: <b>{amount:,.4f} Tokens</b>\n"
        f"Your pick: <b>{'Heads' if choice == 'heads' else 'Tails'}</b>\n\n"
        f"Copy this coin and send it as a message 👇\n\n"
        f"<code>{emoji}</code>\n\n"
        f"<i>Tap the emoji above to copy, then send it!</i>",
        parse_mode="HTML",
...
@dp.message(CoinflipFSM.waiting_emoji)
async def msg_coinflip_emoji(message: Message, state: FSMContext):
    """User sends the coin emoji — check if it matches their pick."""
    text = (message.text or "").strip()
    sent_choice = COIN_EMOJI.get(text)  # "heads" or "tails" or None
    if not sent_choice:
        await message.answer(
            "❌ Send only the coin emoji!\n👑 = Heads   |   🦅 = Tails",
            parse_mode="HTML"
        )
        return
    data = await state.get_data()
    amount = data.get("cf_bet")
    original_choice = data.get("cf_choice")
    if not amount or not original_choice:
        await state.clear()
        await message.answer("Something went wrong. Try again.", parse_mode="HTML")
        return
    user = await db.get_user(message.from_user.id)
    if not user or user["token_balance"] < amount:
        await state.clear()
    ...
    # Win condition: emoji user sent must match what they originally picked
    won_if_result_matches = original_choice
    await state.clear()
    await check_and_process_bonus_eligibility(
    ...
    try:
        await play_coinflip(message, bot, amount, won_if_result_matches)
        await pay_referral_bonus(message.from_user.id, amount)
    except Exception as e:
...
async def msg_game_bet(message: Message, state: FSMContext):
    ...
    if game_key == "coinflip":
        await state.update_data(cf_bet=amount)
        await state.set_state(CoinflipFSM.waiting_emoji)
        await prompt_coinflip(message, amount)
        return
...
async def on_dice_message(message: Message):
    ...
    EMOJI_TO_GAME = {"🎲": "dice", "🪙": "coinflip", "🎯": "highroll"}
