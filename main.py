...
from games.coinflip import prompt_coinflip, play_coinflip, coinflip_side_selected, COIN_VALUE_MAP
...
async def cb_cf_pick(callback: CallbackQuery, state: FSMContext):
    """User picked Heads or Tails — show copyable 🪙 emoji and wait for them to send it."""
    parts = callback.data.split("_")
    ...
    await coinflip_side_selected(callback, amount, choice)


@dp.message(CoinflipFSM.waiting_emoji)
async def msg_coinflip_coin(message: Message, state: FSMContext):
    """User sends 🪙 — Telegram flips it, bot reads the dice value and decides win/lose."""
    # Must be a dice message with 🪙 emoji
    if not message.dice or message.dice.emoji != "🪙":
        await message.answer("Send the 🪙 coin emoji as a message!")
        return
    data = await state.get_data()
    amount = data.get("cf_bet")
    choice = data.get("cf_choice")
    ...
    await state.clear()
    # Wait for Telegram animation to finish (🪙 takes ~2s)
    ...
    try:
        await play_coinflip(message, bot, amount, choice, message.dice.value)
