...
async def cb_loser_board(callback: CallbackQuery):
    ...
    real_losers = await db.get_top_losers(5)
    entries = generate_loser_board(real_losers, 10)
    text = loser_board_text(entries)
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="🔄 Refresh", callback_data="menu_loser_board"))
    builder.row(InlineKeyboardButton(text="🔙 Back", callback_data="menu_leaderboard"))
    try:
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=builder.as_markup())
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup())
