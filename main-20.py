...
async def _finish_pvp_by_ids(resolved: dict, creator_id: int, opponent_id: int, text: str = None):
    if not text:
        text = duel_result_text(resolved)
    winner_id = resolved["winner_id"]
    net_prize = resolved.get("net_prize", resolved["bet"] * 2)
    await db.update_token_balance(winner_id, net_prize)
    await db.add_transaction(winner_id, "pvp_win", net_prize, "completed", "TOKEN")
    loser_id = creator_id if winner_id == opponent_id else opponent_id
    await db.add_transaction(loser_id, "pvp_loss", resolved["bet"], "completed", "TOKEN")
...
async def handle_dice_roll(message: Message):
    ...
    if result is None:
        # Recorded, waiting for opponent
        other_name = duel["opponent_name"] if user_id == duel["creator_id"] else duel["creator_name"]
        await message.reply(
            f"🎲 You rolled: <b>{roll_value}</b>\nWaiting for <b>{other_name}</b> to roll...",
            parse_mode="HTML"
        )
        return
    # Both rolled — resolve
    ...
    if not resolved:
        return
    text = duel_result_text(resolved)
    await _finish_pvp_by_ids(resolved, duel["creator_id"], duel["opponent_id"], text)
